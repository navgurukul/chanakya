exports.up = async function(knex, Promise) {
    ...
    await knex.schema.createTable("englishEnrolment_keys", (table) => {
        table.increments('id').notNullable();
        table.integer('keyId', 10).unsigned().references('id').inTable('enrolment_keys').notNullable()
    ...
    await knex.schema.createTable('englishQuestion_attempts', (table) => {
        table.increments('id').notNullable()
        table.integer('enrolmentKeyId', 10).unsigned().references('id').inTable('englishEnrolment_keys').notNullable();
