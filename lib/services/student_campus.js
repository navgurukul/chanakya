...
module.exports = class studentCampusService extends Schmervice.Service {
  ...
  async studentProgress(data) {
    ...
    const convertedData = [];
    for (const i in data) {
      const student = await Student.query().where('id', data[i].student_id).withGraphFetched({
        partner: true,
        contacts: true,
        enrolmentKey: true,
  ...
  async findAllStudentsByCampusId(campus_id, fromDate, toDate) {
    const { studentCampus, StageTransition, Student } = this.server.models();
    const { studentService } = this.server.services();
    const data = await studentCampus.query().where('campus_id', campus_id);
    const filteredData = [];
    for (var i of data) {
    ...
    for (const i in filteredData) {
      const student = await Student.query()
        .where('id', filteredData[i].student_id)
