...
module.exports = class ExotelService extends Schmervice.Service {
  ...
  async autoCallingToFailedStudent(mobile) {

    if (CONSTANTS.mode !== CONSTANTS.supportedModes.prod) {
      console.log('Not call as the mode is not marked as production. Instead printing it below.');
      console.log("Call is connected");
      return null;
    }

    try {
      await this.axios.post('/Calls/connect', querystring.stringify({
        From: mobile,
        CallerId: CONSTANTS.exotel.callerId,
        Url: `http://my.exotel.com/${CONSTANTS.exotel.sid}/exoml/start_voice/${CONSTANTS.exotel.appId}`
      })).then(() => {
        return true
      })
    } catch (e) {
