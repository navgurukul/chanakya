...
module.exports = class UserService extends Schmervice.Service {
  ...
  async userMobileNumber(user_id, mobile, txn = null) {
    const { Users } = this.server.models();
    const addmobileNo = await Users.query(txn).update(mobile).where('id', user_id);
  ...
  async changePassword(user_id, password, txn) {
    ...
    await Users.query(txn).throwIfNotFound().where({ user_id }).patch({
  ...
  async userUpdate(user_id, Details, txn = null) {
    ...
    if (details.partner_id === 0) {
      ...
      const updatePartnerUser = await Users.query(txn).update(details).where({ id: user_id });
      return updatePartnerUser;
    }
    const updateUser = await Users.query(txn).update(details).where({ id: user_id });
    return updateUser;
  }
