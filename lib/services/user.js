...
module.exports = class UserService extends Schmervice.Service {
  ...
  async createToken(user) {
    const JWTtoken = await JWT.sign({ id: user.id, type: user.type }, this.options.jwtKey, {
      algorithm: 'HS256',
      expiresIn: '7d',
    });
    return JWTtoken;
  ...
  async signUp(Users, txn = null) {
    ...
    if (users) {
      if (user.partnerId === 0) {
        ...
        const createPartnerUser = await User.query(txn).insert(user);
        return createPartnerUser;
      }
      const createUser = await User.query(txn).insert(user);
      return createUser;
    }
  }
