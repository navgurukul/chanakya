...
module.exports = class UserService extends (
  ...
  async sendOTPOnEmail(email) {
    const { exotelService } = this.server.services();

    try {
      const OTP = Math.floor(1000 + Math.random() * 9000); // Generate a 4-digit OTP
      const expirationTime = 180; // OTP expires in 3 minutes (180 seconds)
      // Store the OTP in Redis with an expiration time
      await redisClient.setex(`otp:${email}`, expirationTime, OTP);
      const sendOTP = await sendEmail.sendOTPViaEmail(email, OTP);
      return sendOTP;
    } catch (error) {
      console.error('Error in sendOTPOnEmail:', error);
  ...
  async verifyOTP(email, userOTP) {
    try {
      //  if the OTP length is not equal to 4, return an error
      if (userOTP.length !== 4) {
        return { isValid: false, message: 'Only a 4 digit OTP is allowed.' };
      }
