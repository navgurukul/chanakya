...
module.exports = class UserService extends Schmervice.Service {
  ...
  async googleLogin(idToken, txn = null) {
    ...
    const userObj = {
      ...
      mailId: response.payload.email.split('@')[0],
  ...
  async userMobileNumber(userId, mobile, txn = null) {
    const { Users } = this.server.models();
    const addmobileNo = await Users.query(txn).update(mobile).where('id', userId);
    const [ user ] = await Users.query(txn).where('id', userId);
    return {
      addmobileNo,
      user
    };
