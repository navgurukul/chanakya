...
module.exports = class PartnerService extends Schmervice.Service {
    ...
    // It gives the name of Paperset to particular partners assessment. 
    async paperSet(questionSetId, txn= null) {
        const { PartnerAssessment } = this.server.models()
        let paper = await PartnerAssessment.query(txn).where({questionSetId: questionSetId})
        return paper[0]
    }
    ...
    async create(details, txn=null) {
        const { Partner } = this.server.models();
        let partners = await this.findByName(details.name, txn);
    ...
    async recordAssessmentDetails(partner, assessmentId, csvUrl, txn=null) {
        ...
        let questionSet = await assessmentService.getQuestionsOfQuestionSet(partnerAssessment.questionSetId);
