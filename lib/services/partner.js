...
internals.csvRowAttrsSchema = Joi.object({
  ...
  current_status: Joi.string().valid(..._.keys(CONSTANTS.studentDetails.current_status)).allow(null),
...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async createAssessment(partner, assessmentName, txn = null) {
    ...
    const { questionPaperUrl, answer_key_url } = await createTestAndAnswerKeyPDF(questions,
      partner.name, assessmentName);
  ...
  async recordAssessmentDetails(partner, assessmentId, csvUrl, txn = null) {
    ...
    const questionSet = await assessmentService
      .getQuestionsOfQuestionSet(partnerAssessment.question_set_id);
  ...
  async progressMade(partner_id) {
    ...
    const students =  await Student.query().eager({contacts: true})
      .where('partner_id', partner_id).orderBy('created_at', 'desc')
    ...
    _.each(students, (student) => { 
        ...
        if(stage == "finalisedTravelPlans" || stage == "deferredJoining") {
          convertedData["Selected for Navgurukul One-year Fellowship"][stage].push(data)
        }
        if(stage == "pendingAlgebraInterview" || stage == "pendingCultureFitInterview" || stage == "pendingEnglishInterview" || stage == "pendingParentConversation") {
        ...
        if(stage == "testFailed" || stage == "englishInterviewFail" || stage == "algebraInterviewFail") {
