...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async meraki_link(partner_id) {
    const { Partner } = this.server.models();
    const merakiLink = `${CONSTANTS.meraki_link}${partner_id}`;
  ...
  async student_progressMade_Cards(partner_id) {
    const studentProgressMade = await this.student_progressMade(partner_id);
    const convertedData = {
      Python: {
        M1: [],
        M2: [],
    ...
    for (var student of studentProgressMade) {
      for (var card of Object.keys(convertedData)) {
        for (var stage of Object.keys(convertedData[card])) {
