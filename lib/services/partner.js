...
module.exports = class PartnerService extends Schmervice.Service {
  async checkPartnerEmailExist(emailList){
    const { PartnerUser } = this.server.models();
    emailList = emailList.map(e=>e.email)
    const emailExist = await PartnerUser.query().whereIn("email",emailList);
  ...
  async updatePartnerUser(emails){
    ...
    for (var i of emails){
      if(i.id){
        ...
        const emailExist = await PartnerUser.query().where("email",i.email);
          if (emailExist.length==0){
            await PartnerUser.query().insert({email:i.email,partner_id:i.partner_id})
          }else{
            if (existingEmails.length==0){
              existingEmails=i.email
            }else{
              existingEmails=existingEmails+", "+i.email
            }
          }          
        }        
    }
    ...
    if (existingEmails.length!==0){
      return {
        error:true,
        message: `email alreay exist : ${existingEmails}`,
      }
    }else{
      return {
        "status": "success",
        "message": "partner user emails updated"
      }
    }
  }
  async findById(id, eagerExpr = null, txn = null) {
    const { Partner } = this.server.models();
    let query = Partner.query(txn).throwIfNotFound().findById(id).eager("partnerUser");
  ...
  async findAll(txn) {
    const { Partner } = this.server.models();
    const partner = await Partner.query(txn).eager("partnerUser");
    return partner;
  }
  ...
  async create(details, txn = null) {
    ...
    if (!validateSlug.test(details.slug)) {
      return {
        error: true,
        message:`Invalide slug, set slug as minimum small letter character and number.`,
    ...
    if (partners.length > 0) {
      return {
        error: true,
        message:`Duplicate entry for partner with name ${partners[0].name} already exists.`,
      }
      // throw Boom.conflict(
