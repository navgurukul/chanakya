...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async create(details, txn = null) {
    ...
    console.log(withDistricts);
    await Partner.query(txn).insert(withDistricts);
    return Partner.query().where({ slug: details.slug });
  }
  ...
  async createAssessment(partner, assessmentName, txn = null) {
    ...
    const {
      questionPaperUrl,
      answer_key_url,
    } = await createTestAndAnswerKeyPDF(
      questions,
      partner.name,
      assessmentName
    );
  ...
  async progressMade(partner_id) {
    ...
    const convertedData = {
      "Selected for Navgurukul One-year Fellowship": {
        ...
        finallyJoined: [],
        deferredJoining: [],
        pendingTravelPlanning_Unreachable: [],
        pendingTravelPlanning_NotResponding: [],
        pendingTravelPlanning_BecameDisinterested: [],
        pendingTravelPlanning_WillJoinIn: [],
        pendingTravelPlanning_TicketBooked: [],
        pendingTravelPlanning_NeedFinancialSupport: [],
        pendingTravelPlanning_TalkToParents: [],
      },
      "Need Action": {
    ...
    _.each(students, (student) => {
      ...
      if (
        ...
        stage === "pendingTravelPlanning_NotResponding" ||
        stage === "pendingTravelPlanning_BecameDisinterested" ||
        stage === "pendingTravelPlanning_WillJoinIn" ||
        stage === "pendingTravelPlanning_TicketBooked" ||
        stage === "pendingTravelPlanning_NeedFinancialSupport" ||
        stage === "pendingTravelPlanning_TalkToParents" ||
