...
const { createTestAndAnswerKeyPDF } = require('../../lib/helpers/pdfGenerator');
...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getQuestionsOfQuestionSet() {
    const { assessmentService } = this.server.services();
    const { questions, questionSet } = await assessmentService.createQuestionSetForPartner(); // assuming this returns both
