...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async inFormTestResult(key, total_marks) {
    ...
    const { feedbackService, ownersService, studentService } =
      this.server.services();
    const patchedDetail = studentService.findById(Key.student_id);
    console.log(patchedDetail, "patchedDetail");
    if (
      patchedDetail[0].length > 0 &&
      patchedDetail[0].stage === "pendingEnglishInterview"
    ) {
