...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async generateAssessmentQuestions() {
    ...
    const school_test='School of BCA'

    const questionSet = [];
    if(school_test=='School of BCA'){
      console.log('55555')
      let schoolQuestions = await Question.query().where('school_test',school_test).withGraphFetched('options');
      // Select a random set of 5 questions
      schoolQuestions = schoolQuestions.sort(() => 0.5 - Math.random()).slice(0, 5);
      questionSet.push(...schoolQuestions);
      // console.log(schoolQuestions,'>>>>>>53333')
    }else{
