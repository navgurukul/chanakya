...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    // async recordTypeOfTest(student, typeOfTest, txn=null){
    //     const { Student } = this.server.models();
    //     await Student.query(txn).insert({
    ...
    async patchStudentDetails(key, details={}, txn=null) {
        const { Student } = this.server.models();
        // record students type of test on students table
        if (details.stage){
            details.typeOfTest = "onlineTest";
        }
