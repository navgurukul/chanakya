...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    async initialize() {
        ...
        let currentVersion = await this.findCurrentTestVersion();
        let questions = await Question.query()
            .whereIn('id', currentVersion.questionIds).eager('options');
    ...
    /* TODO: Move the versions related code to a new service file. Will be easier to manage. */
    async findAllTestVersions() {
        const { TestVersion } = this.server.models();
        return await TestVersion.query();
    }

    async findCurrentTestVersion() {
        const { TestVersion } = this.server.models();
        let version = await TestVersion.query().findOne({ current: true });
        return version;
    }

    async createNewTestVersionAndMarkCurrent(version) {
        const { TestVersion } = this.server.models();
        let oldVersion = await this.findCurrentTestVersion();

        // create a new version with the flag of current
        version = _.extend(version, {current: true});
        let currentVersion = await TestVersion.query().insertGraph(version);

        // disable the current flag on the older current version
        oldVersion.current = false;
        await TestVersion.query().upsertGraph(oldVersion);
    ...
    getAttempts(answers, key, questions) {
        ...
        let attempts = _.map(_.pairs(answers), (answer) => {
            ...
            if (attempt.question.type == CONSTANTS.questions.types.integer) {
                attempt.textAnswer = answer[1] == null ? null: String(answer[1]).trim();
            } else {
                attempt.selectedOptionId =  answer[1] == null ? null : Number(answer[1]);
            }
    ...
    async recordOfflineStudentAnswers(studentDetails, answers, questionSet, txn=null) {
        ...
        let student = await studentService.create(mobile, "completedTest", null, studentDetails)
