...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    async informTestResult() {
        ...
        _.each(students, async (student) => {
            ...
            let stage;
            // According to the Cut off marks for female and male change students stage and inform their test result.
            if (studentDetail[0].student.gender == CONSTANTS.studentDetails.gender.female && studentDetail[0].totalMarks >= CONSTANTS.testCutoffMale[testVersion]) {
                stage = 'testPassed';
            }else {
                stage= 'testFailed';
            }
            
            if (studentDetail[0].student.gender == CONSTANTS.studentDetails.gender.male && studentDetail[0].totalMarks >= CONSTANTS.testCutoffFemale[testVersion]){
                stage = "testPassed";
            } else {
                stage = "testFailed";
            }
            sendSMSPromises.push(this.patchStudentDetails(studentDetail[0], {
                stage,
            }))
        });
