...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    async recordStageTranisiton(student, toStage, txn=null){
        ...
        // send sms after recording the stage transitions if stage is not related to the online/offline test result. 
        if (toStage != "testPassed" && toStage != 'testFailed'){
    ...
    async informTestResult() {
        ...
        let students = await Student.query().where('stage', 'completedTestWithDetails')
        let sendSMSPromises = []

        _.each(students, async (student) => {
            let key = await EnrolmentKey.query().where('studentId', student.id);
            let studentDetail = await EnrolmentKey.query().eager({
                student: {
                    contacts: true,
                },
                questionSet: {
            ...
            let testVersion = studentDetail[0].questionSet.testVersion.name;
            let mobile = studentDetail[0].student.contacts[0].mobile;
            let stage;
            const templateContext = {
                studentDetail
            }
            
            if (studentDetail[0].student.gender == 2 && data[0].totalMarks >= CONSTANTS.testCutoffMale[testVersion]) {
                stage = 'testPassed';
            }else {
                stage= 'testFailed';
            }

            if (studentDetail[0].student.gender == 1 && data[0].totalMarks >= CONSTANTS.testCutoffFemale[testVersion]){
                stage = "testPassed";
            } else {
                stage = "testFailed";
            }

            if ( exotelService.hasTemplateForStage(stage) == true ) {
                sendSMSPromises.push(exotelService.sendSMS(mobile, stage, templateContext))
            }
        });
