...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async getQuestionsOfQuestionSet(question_set_id, txn = null) {
    ...
    let questions = await Question.query(txn).findByIds(questionSet.question_ids).eager('options');
  ...
  async getQuestionSetForEnrolmentKey(key, txn = null) {
    ...
    if (key.question_set_id) { // the question set has already been created
  ...
  async inFormTestResult(key, total_marks) {
    const { EnrolmentKey } = this.server.models();
    const Key = key; const totalMark = total_marks;
  ...
  async recordStudentAnswers(key, answers, txn) {
    ...
    const ansDiff = _.difference(answersquestion_ids, _.map(questions, (question) => question.id));
    if (ansDiff.length !== 0) {
  ...
  async addOrUpdateWhatsappNumber(student_id, whatsappNum, txn = null) {
    ...
    const contacts = await Contact.query(txn).where({ mobile: whatsappNum, student_id });
  ...
  async patchStudentDetails(key, Details = {}, gender, txn = null) {
    ...
    if (details.whatsapp) {
      ...
      await this.addOrUpdateWhatsappNumber(key.student_id, whatsapp, txn);
    ...
    const patchStudentDetail = await Student.query(txn).patch(details).where({ id: key.student_id });
    return patchStudentDetail;
  }
