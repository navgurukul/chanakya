...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    async informTestResult() {
        const { StageTransition, Contact } = this.server.models();
        const { exotelService } = this.server.services();
        let studentStages = await StageTransition.query().where('toStage','testPassed').orWhere('toStage', 'testFailed')
        let sendSMSPromises = [];
        _.each(studentStages, async (studentStage) => {
            let contact = await Contact.query().where('studentId', studentStage.studentId).andWhere('isWhatsapp',true)
                console.log(contact)
            if(exotelService.hasTemplateForStage(studentStage.toStage) == true){
                let contact = await Contact.query().where('studentId', studentStage.studentId).andWhere('isWhatsapp', true)
                if (contact.length != 0) {
                    const templateContext = {
                        contact
                    }
                    sendSMSPromises.push(exotelService.sendSMS(contact[0].mobile, studentStage.toStage, templateContext))
                }
            }
        })
        return await Promise.all(sendSMSPromises)
