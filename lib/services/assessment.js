...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    async patchStudentDetails(key, details, txn=null) {
        const { Student, Contact, StageTransition } = this.server.models();
        
        // add students stage on stage_transition table once student complete his test.
        if(details.stage != null){
            await StageTransition.query(txn).where({studentId: key.studentId})
            .insert({
        ...
        if (whatsapp) {
            let contacts = await Contact.query(txn).where({ mobile: whatsapp, studentId: key.studentId });
