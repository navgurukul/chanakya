...
module.exports = class AssessmentService extends Schmervice.Service {
    ...
    async patchStudentDetails(key, details, txn=null) {
        ...
        if( condition ){
            await Contact.query(txn).insert({ mobile: studentWhatsapp, studentId: key.studentId, isWhatsapp: true });
        }else{
            await Contact.query(txn).patch({ isWhatsapp: true }).where({ studentId: key.studentId })
        }
