...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async generateAssessmentQuestions(school_test) {
    ...
    if (school_test !== undefined) {
      ...
      const numberObj = {
        'Percentage': { 1: 1, 2: 1, 3: 2 }, //1, 2, 2  => easy, med, hard
        'Unitary Method': { 1: 1, 2: 2, 3: 0 }, //2, 2, 0  => easy, med, hard
        'Algebra': { 1: 1, 2: 2, 3: 1 }, //1, 2, 2 => easy, med, hard
        'Profit and Loss': { 1: 1, 2: 1, 3: 1 } //1, 1, 1 => easy, med, hard
      };
  ...
  async inFormTestResult(key, total_marks) {
    ...
    let school;
    if (totalMark >= (cutOff[testVersion] || 13)) {
  ...
  async ShowTestResult(Key, txn = null) {
    ...
    if (key.length && questionSet) {
      ...
      const Result = key[0].total_marks >= (cutOff[TestVersion] || 13) ? 'Passed' : 'Failed';
      return {
        Result,
