...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async sendEnglishTestLink(mobile, stage, keyId, key, txn = null) {
    ...
    // recorde the students enrolment keyId into englishEnrolment_key table for storing
    // students english test marks, and for verifying validate enrolnment key.
    await EnglishEnrolmentKey.query(txn).insert({
      keyId,
    });

    await exotelService.sendSMS(mobile, stage, key); // sendEnglishTestLink

    // change student stage onces send the english test link.
    const enrolmentkey = await this.validateEnrolmentKey(key);
  ...
  async informTestResult() {
    ...
    const sendSMSpromises = await Promise.all(sendSMSPromises);
  ...
  async patchStudentDetails(key, Details = {}, txn = null) {
    ...
    if (!details.length && key.student.stage === 'pendingEnglishInterview' && key.student.contacts) {
      await this.sendEnglishTestLink(key.student.contacts[0].mobile, key.student.stage,
        key.id, key.key);
    }
