...
const Milestones = require('../config/index');
...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async recordStageMilestoneTrabsition(student, to_stage, gender, transition_done_by) {
    const { InterviewTransition, MilestoneTransition } = this.server.models();
    const { exotelService } = this.server.services();

    const milestoneStages = Object.keys(Milestones.campusStageOfLearning);
    const stageLastIndex = Object.keys(Milestones.allStages).indexOf('testFailed');
    const interviewStages = Object.keys(Milestones.allStages).slice(0, stageLastIndex + 1);

    const created_at = new Date().toISOString();
    if (interviewStages.includes(to_stage)) {
      await InterviewTransition.query()
        .insert({
    ...
    } else if (milestoneStages.includes(to_stage)) {
      await MilestoneTransition.query()
        .insert({
          student_id: student.id,
          from_stage: student.stage,
          to_stage,
    ...
    if (gender == 2) {
      to_stage = 'closeBoysAdmission';
    }
    if (exotelService.hasTemplateForStage(to_stage) === true) {
      if (!student.contacts) {
      ...
      _.each(student.contacts, (contact) => {
        ...
        sendSMSPromises.push(exotelService.sendSMS(contact.mobile, to_stage, templateContext));
      });
  ...
  async newPatchStudentDetails(enrolmentKey, details) {
    const newDetails = details;
    const { Student } = this.server.models();
    if (newDetails.stage) {
      const lastUpdated = await this.recordStageMilestoneTrabsition(
        enrolmentKey.studentStatus[0],
        details.stage,
        enrolmentKey.studentStatus[0].gender,
    ...
    try {
      if (!newDetails.name) {
        ...
        return enrolmentKey.studentStatus;
      }
  ...
  async recordStageTranisiton(student, to_stage, gender,transition_done_by, txn = null) {
  ...
  async ShowTestResult(Key, txn = null) {
    ...
    if (key.length && questionSet) {
      ...
      const Result = key[0].total_marks >= cutOff[TestVersion] ? 'Passed' : 'Failed';
      return {
        Result,
