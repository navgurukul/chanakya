...
module.exports = class EmailReportService extends Schmervice.Service {
  ...
  async getEmailReportsById(id, txn = null) {
    ...
    const emailReport = await EmailReport.query(txn).where('partner_id', partnerId);
    return emailReport;
  }
  ...
  async getPartners() {
    const { EmailReport } = this.server.models();
    const data = await EmailReport.query().select('partner_id', 'emails', 'report', 'repeat');
    return data;
  }
  ...
  async sendEmailByPartnerId(partner_id) {
    try {
      ...
      res.link =
        partner_id != null ? `https://admissions.navgurukul.org/partner/${partner_id}` : '';
