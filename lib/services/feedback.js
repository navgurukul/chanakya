...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async assingeFeedbackWork(payload, txn = null) {
    const { Feedback } = this.server.models();
    const details = payload;
    const allreadyAssinged = await Feedback.query(txn).where('studentId', details.studentId)
      .andWhere('student_stage', details.student_stage);

    if (allreadyAssinged) {
      const updateAssingedWork = await Feedback.query(txn).update(details)
        .where('studentId', details.studentId)
        .andWhere('student_stage', details.student_stage);
      return updateAssingedWork;
    }
    const assingedWork = await Feedback.query(txn).insert(details);
    return assingedWork;
  }
};
