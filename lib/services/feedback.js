...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async updateFeedback(feedback, student_id, txn = null) {
    ...
    if (feedbacks.state) {
      const ownerId = await Student.query()
        .where('id', student_id)
        .select('current_owner_id');
      await Student.query()
        .update({ current_owner_id: null })
        .where('id', student_id);
  ...
  async updateOrInsertByStudentStage(feedback, studentId, txn = null) {
    const { Feedback } = this.server.models();
    const feedbacks = feedback;
    const Count = await Feedback.query(txn)
  ...
  async joinedCampusDate(studentId, txn) {
    ...
    for (const index in leaveEnd) {
      if (leaveStart[index] != undefined) {
        const startDate = leaveStart[index].created_at;
        const endDate = leaveEnd[index].created_at;
        totalLeaveDays += await studentService.leaveDaysCalculate(
          startDate,
          endDate,
        );
      }
