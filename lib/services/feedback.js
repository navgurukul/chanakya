...
module.exports = class FeedbackService extends Schmervice.Service {
  async addFeedback(student, user, feedback, txn = null) {
    ...
    const existingStage = await Feedback.query(txn).where('student_id', Student.id)
    ...
    if (existingStage.length) {
      if (existingStage[0].user_id) {
        const addfeedback = await Feedback.query(txn).update(details)
          .where('student_id', Student.id).andWhere('student_stage', feedbacks.student_stage);
        return addfeedback;
      }
  ...
  async updateFeedback(feedback, student_id, txn = null) {
    ...
    const update = await Feedback.query(txn).update(feedbacks).where('student_id', student_id)
      .andWhere('student_stage', feedbacks.student_stage)
  ...
  async sendSMSto_assignUser(details, txn = null) {
    ...
    const [student] = await Student.query(txn).where('id', details.student_id).eager({
      feedbacks: {
        assignUser: true,
      },
    });
  ...
  async assignFeedbackWork(payload, txn = null) {
    ...
    const allreadyAssigned = await Feedback.query(txn).where('student_id', details.student_id)
  ...
  async alltransitionsWithFeedback(student_id, txn = null) {
    ...
    const transitions = await StageTransition.query(txn).where('student_id', student_id).eager({
      feedbacks: true,
    });
  ...
  async myTaskReport(user, txn = null) {
    const { Feedback } = this.server.models();
    const User = user;
    const tasks = await Feedback.query(txn).where('to_assign', User).eager({ student: true });
    return tasks;
  }
  ...
  async myAssignReport(user, txn = null) {
    const { Feedback } = this.server.models();
    const User = user;
    const tasks = await Feedback.query(txn).where('who_assign', User).eager({ student: true });
    return tasks;
  }
  ...
  async getSMSPromises(deadlineShift, templateName) {
    ...
    if (templateName == 'informToCompleteTheTest') {
      const smsStudents = await Student.query().eager({
        contacts: true,
        enrolmentKey: true
      }).where('stage', 'basicDetailsEntered').where('created_at', '>', lowerLimit).where('created_at', '<', upperLimit);
      ...
      _.each(smsStudents, (student) => {
        ...
        const mobile = student.contacts[0].mobile;
        sendSMSPromises.push(exotelService.sendSMS(mobile, templateName, templateContext))
      })
    } else {
      const smsUsers = await Feedback.query().eager(relation).whereNull('state').whereNotNull('to_assign')
  ...
  async addaudio_recordingUrl(Ids, url, txn = null) {
    ...
    const { Feedback } = this.server.models();
    const existingStage = await Feedback.query(txn).where('student_id', studentId)
    ...
    if (existingStage.length) {
      if (existingStage[0].user_id) {
        ...
        const addAudioUrl = await Feedback.query(txn).update(details)
          .where('student_id', studentId).andWhere('student_stage', student_stage);
        return addAudioUrl;
      }
      const addAudioUrl = await Feedback.query(txn).update(details)
        .where('student_id', studentId).andWhere('student_stage', student_stage);
  ...
  async pendingInterview(user) {
    ...
    const studentDetails = await Feedback.query().eager({
      ...
      .where('to_assign', user).andWhere('feedback', null);
