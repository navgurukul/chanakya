    ...
    if (school_id.length !== 0) {
      if (school_id[0].school_id !== 1) {
        transitions = await StudentStage.query(txn)
          .where('student_id', student_id)
          .withGraphFetched({
            feedbacks_school: true,
          });
        ;
      }
    }
    _.each(transitions, (transition, index) => {
      if (transition.feedbacks_school) {
        transitions[index].feedbacks_school = transition.feedbacks_school.feedback;
      } else {
        transitions[index].feedbacks_school = null;
        delete transitions[index].feedbacks_school;
      }
    });
    return transitions;
  }
