...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async all_school_feedback(student_id, txn = null) {
    const { StudentStage, StudentSchool } = this.server.models();
    let transitions;
    const school_id = await StudentSchool.query().select('school_id').where('student_id', student_id)
    if (school_id.length !== 0) {
      if (school_id[0].school_id !== 1) {
        transitions = await StudentStage.query(txn)
          .where('student_id', student_id)
          .withGraphFetched({
            feedbacks: true,
    ...
    const formattedData = transitions.map(item => ({
      id: item.id,
      student_id: item.student_id,
      from_stage: item.from_stage,
      to_stage: item.to_stage,
      created_at: item.created_at,
      transition_done_by: item.transition_done_by,
      feedback: item.feedbacks ? item.feedbacks.feedback : null,
      state: item.feedbacks ? item.feedbacks.state : null,
      who_assign: item.feedbacks ? item.feedbacks.who_assign : null,
      to_assign: item.feedbacks ? item.feedbacks.to_assign : null,
      audio_recording: item.feedbacks ? item.feedbacks.audio_recording : null,
      deadline: item.feedbacks ? item.feedbacks.deadline_at : null,
      finished_at: item.feedbacks ? item.feedbacks.finished_at : null,
      last_updated: item.feedbacks ? item.feedbacks.last_updated : null,
      time: item.feedbacks ? item.feedbacks.created_at : null
    }));
