...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async getSMSPromises(deadlineShift, templateName) {
    ...
    // get current hours
    const cur_deadline = date.getHours();
    ...
    let lowerLimit = new Date(date).setHours(cur_deadline + deadlineShift);
    let upperLimit = new Date(date).setHours(cur_deadline + deadlineShift + 1);
