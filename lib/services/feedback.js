...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async informPendingMobilizationWorkToAssignUser() {
    const { Feedback } = this.server.models();
    const { exotelService } = this.server.services();
    const users = await Feedback.query().eager({
      student: true,
      assignUser: true,
    }).whereNull('state').whereNotNull('toAssign');

    const sendSMSPromises = [];
    _.each(users, (user) => {
      const { deadline } = CONSTANT.feedbackableStagesData[user.student.stage];
      const diff = new Date().getTime() - new Date(user.deadlineAt).getTime();
      ...
      if (remainigTime <= CONSTANT.miniRemaingDeadlineTime && remainigTime > 0) {
        sendSMSPromises.push(exotelService.sendSMS(user.assignUser.mobile, 'deadlineMessage', templateContext));
      } else if (remainigTime < 0) {
        sendSMSPromises.push(exotelService.sendSMS(user.assignUser.mobile, 'deadlineEndMessage', templateContext));
      }
    });
