  ...
  async alltransitionsWithFeedback(student_id, txn = null) {
    ...
    if (school_id[0].school_id !== 1) {
      transitions = await StudentStage.query(txn)
        ...
        transitions_programming = await StageTransition.query(txn)
        .where('student_id', student_id)
        .withGraphFetched({
          feedbacks: true,
        });

      transitions.push({ transitions_for_programming: transitions_programming })
    } else {
      transitions = await StageTransition.query(txn)
        .where('student_id', student_id)
        .withGraphFetched({
          feedbacks: true,
        });
