...
module.exports = class OwnersService extends Schmervice.Service {
  async newOwner(owner) {
    ...
    return await Owners.query().where({ id }).withGraphFetched('user');
  }
  async updateOwner(owner, id) {
    ...
    return await Owners.query().where('id', id).withGraphFetched('user');
  }
  async findall(query) {
    const { Owners } = this.server.models();
    if (query) {
      return await Owners.query().where(query).withGraphFetched('user');
    }
    return await Owners.query().withGraphFetched('user');
  }
  async findById(id) {
    const { Owners } = this.server.models();
    return await Owners.query().where('id', id).withGraphFetched('user');
  ...
  async pendingInterviewUpdate(ownerId) {
    ...
    await Owners.query().update({ pending_interview_count: owner.length }).where('id', ownerId);
  }
  ...
  async autoAssignFeat(details, feedbackService, stage, student_id) {
    ...
    if (assing_to.length > 0) {
      ...
      await Student.query().update({ current_owner_id: assing_to[0].id }).where('id', student_id);
