...
module.exports = class TestVersioningService extends Schmervice.Service {
  ...
  async createAndMarkAsCurrent(versionName, questionIds, buckets, txn = null) {
    const { Question, QuestionBucket, TestVersion } = this.server.models();
    ...
    if (questions.length !== questionIds.length) {
      logger.error(
        JSON.stringify({
          error: true,
          message: 'All the question_ids given are not valid.',
        })
      );
    ...
    const dbBuckets = await QuestionBucket.query(txn)
      .whereIn('id', bucket_ids)
      .withGraphFetched('choices');
    if (dbBuckets.length !== bucket_ids.length) {
      logger.error(
        JSON.stringify({
          error: true,
          message: 'All the IDs of the bucket in the bucket object are not valid.',
        })
      );
      ...
      _.each(b.choiceIds, (cId) => {
        ...
        if (dbChoice.length <= 0) {
          logger.error(
            JSON.stringify({
              error: true,
              message: 'All the choice IDs are not valid.',
            })
          );
  ...
  async getQuestions(version) {
    ...
    const questions = await Question.query()
      .whereIn('id', version.data.questionIds)
      .withGraphFetched('options');
    versionQuestions.questions = [..._.flatten(questions)]
    ...
    _.each(version.data.buckets, (b) => {
      const promise = QuestionBucket.query()
        .findById(b.bucketId)
        .then((bucket) => {
          versionQuestions.buckets.push({ name: bucket.name, id: bucket.id, choices: [] });
          return QuestionBucketChoice.query().whereIn('id', b.choiceIds).withGraphFetched('bucket');
        })
        .then((choices) => {
          const newPromises = [];
          _.each(choices, (choice) => {
            const p = Question.query()
              .whereIn('id', choice.question_ids)
              .withGraphFetched('options')
              .then((ques) => {
