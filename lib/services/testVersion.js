...
module.exports = class TestVersioningService extends Schmervice.Service {
  ...
  async createAndMarkAsCurrent(versionName, questionIds, buckets, txn = null) {
    ...
    const dbBuckets = await QuestionBucket.query(txn).whereIn('id', bucket_ids).withGraphFetched('choices');
  ...
  async getQuestions(version) {
    ...
    //   questions.forEach(CONSTANTS.questions.topics, (topic) => {
    //   versionQuestions.withoutChoices[topic] = {
    //     easy: [],
    //     medium: [],
    //     hard: [],
    //   };
    ...
    //         const p = Question.query().whereIn('id', choice.question_ids).withGraphFetched('options')
    //           .then((ques) => {
