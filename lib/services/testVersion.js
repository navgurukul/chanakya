...
module.exports = class TestVersioningService extends Schmervice.Service {
  ...
  async findCurrent(txn) {
    const { TestVersion } = this.server.models();
    const version = await TestVersion.query().findOne({ current: true });
    return version;
  }
  ...
  async getQuestions(version) {
    ...
    const questions = await Question.query().whereIn('id', version.data.questionIds).eager('options');
        ...
        .then((choices) => {
          ...
          _.each(choices, (choice) => {
            const p = Question.query().whereIn('id', choice.questionIds).eager('options')
              .then((ques) => {
                const bucketIndex = _.findIndex(versionQuestions.buckets, (b) => {
