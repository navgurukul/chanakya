...
module.exports = class SchoolService extends Schmervice.Service {
  ...
  async schoolsAvailableInCampuses(campus_id) {
    const { CampusSchool } = this.server.models();
    try {
      const store = await CampusSchool.query().select('school_id').where('campus_id', campus_id);
      return store;
    } catch (err) {
      return err;
    }
  ...
  async studentsAvailable_school(school_id) {
    ...
    try {
      ...
      if (school_id == 1) {
        ...
        student = (await StudentSchool.query().where('school_id',school_id)).length1
      }
