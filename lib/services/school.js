...
module.exports = class SchoolService extends Schmervice.Service {
  ...
  async students_school_post(studentId, schoolId) {
    ...
    try {
      console.log(studentId, schoolId, 'student_id, school_id');
      const existingRecord = await StudentSchool.query().findOne({ student_id: studentId });
      let result;

      if (existingRecord) {
        result = await StudentSchool.query()
          .patchAndFetchById(existingRecord.id, { school_id: schoolId });
      } else {
        result = await StudentSchool.query()
      ...
      console.error('Error in students_school_post:', err);
      throw err;
    }
  }
