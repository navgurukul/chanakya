...
module.exports = class StudentService extends Schmervice.Service {
    ...
    async findPartnerById(questionSetId, paperSet, txn=null){
        // Get the students information using question set ID.
        const { Student, EnrolmentKey, PartnerAssessment } = this.server.models();
        return Student.query(txn).where('enrolment_keys.questionSetId', questionSetId).join('enrolment_keys','students.id', '=', 'enrolment_keys.studentId');
    }
    ...
    async findStudentsByStageTransition(toStage, whereClause={}, whereNotClause={}, txn=null) {
        ...
        let q = StageTransition.query(txn).joinRelation('student')
    ...
    async studentStageNotChangeSince(student) {
