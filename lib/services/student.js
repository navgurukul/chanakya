...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async studentsDataForDashBoard(
    ...
    if (searchOwnerName != "") {
      ...
      for (let n in searchOwner) {
        searchOwner[n] = searchOwner[n].id;
      }
      query += `.whereIn("id", searchOwner)`;
    }
  ...
  async markDuplicateOfSameStudent(number, name, enrolmentKey) {
    var { Contact, Student } = this.server.models();
    ...
    contacts = contacts
      .filter((contact) => contact.student.name === name)
      .map((contact, index) => {
        indexMain = enrolmentKey.student_id === contact.student.id ? index : 0;
        return contact;
      })
      .map((contact, index) => {
        indexMain = contact.student.stage.includes("pending")
          ? index
          : indexMain;
        return contact;
      })
