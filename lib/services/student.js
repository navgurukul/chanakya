...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async studentStatus(mobileNumber, txn = null){
    ...
    _.each(contact, (studetns) => {
      const { transitions, student, enrolmentKey } = studetns;
      const studentStatus = {};
      const updatedStage = transitions[transitions.length - 1];
      if (updatedStage.toStage === "pendingEnglishInterview" && enrolmentKey.totalMarks !== null) {
        ...
        studentStatus.linkForEnglishTest = `http://join.navgurukul.org/k/${enrolmentKey.key}`;
        ...
        studentStatus.City = student.city;
        studentStatus.State = CONSTANTS.studentDetails.states[student.state];
        studentStatus.Marks = enrolmentKey.totalMarks !== null ? enrolmentKey.totalMarks: 'Online test is not given yet';
        studentStatus.Gender = _.invert(CONSTANTS.studentDetails.gender)[student.gender];
