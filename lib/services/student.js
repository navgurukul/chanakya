...
module.exports = class StudentService extends Schmervice.Service {
  async findJobKabLagega(joindate, milestone, totalMilestone = 18) {
    let milestones = [
      "M1",
      "M2",
      "M3",
    ...
    if (!milestones.includes(milestone) || !date) {
      return null;
    }
    let todayDate = new Date();
    let daysPassed = date.subtract(todayDate, joindate).toDays();
    let kitneDinLagenge = (daysPassed / milestone.slice(1)) * totalMilestone;
    console.log(
    ...
    return date.format(date.addDays(joindate, kitneDinLagenge), "YYYY-MM-DD");
  }
    ...
    if (Stage) {
      if (FromDate && ToDate) {
        ...
        const promises = _.map(array, async (student) => {
          ...
          const date = student.studentCampus
            ? student.studentCampus.joined_campus_on
            : null;
          student.jobKabLagega = await this.findJobKabLagega(
            date,
            student.stage
          );
          delete student.studentCampus;
          student.campus =
            student.campus.length > 0 ? student.campus[0].campus : null;
  ...
  async getStudentById(id, txn = null) {
    ...
    const date = student[0].studentCampus.joined_campus_on
      ? student[0].studentCampus.joined_campus_on
      : null;
    student[0].jobKabLagega = await this.findJobKabLagega(
      date,
      student[0].stage
    );
    delete student[0].studentCampus;
    student[0].campus =
