...
module.exports = class StudentService extends Schmervice.Service {
  async deleteStudentDetails(student_id){
    ...
    // await 
    await StudentSchool.query().delete().where({student_id})
  ...
  async deleteTransition(transitionId, txn) {
    const { StageTransition, Student, Feedback, School,StudentSchool } = this.server.models();
    ...
    const storeId = await StageTransition.query().select('student_id').where("id", transitionId)
    if (storeId.length == 0) {
      return { success: false };
    }
    const d = await StageTransition.query(txn).where("id", transitionId).delete();
    const identifyStage = await StageTransition.query().select('school', 'id').where("student_id", storeId[0].student_id).orderBy('id', 'asc')
    if (identifyStage.length > 0) {
      const schoolName = identifyStage[identifyStage.length - 1].school
      const getSchoolId = await School.query().select('id').where('name','ILIKE',schoolName)
      if (getSchoolId.length > 0) {
        const schoolId = getSchoolId[0].id;
        const updatedRows = await StudentSchool.query().update({ school_id: schoolId }).where({ student_id: storeId[0].student_id });
      }
    }
    return { success: true };
  }
  ...
  async create(stage, incomingCall = null, Details = {}, txn = null) {
    ...
    // add a transition
    details.transitions = [{ to_stage: stage,school:'School Of Programming' }];
    const studentDetails = await Student.query(txn).insertGraph(details);
    return studentDetails;
  }
  ...
  async getGithubUrl(student_id){
    let gitHubSecret = CONSTANTS.gitHub.gitHubSecret
    let school_id = CONSTANTS.gitHub.school_id
    let gitHubUrl = "https://education.github.com/student/verify/generate?school_id=" + school_id + "&student_id=" + student_id + "&secret_key=" + gitHubSecret;
    try{
      const getUrl = await axios.get(gitHubUrl).then((res)=>{
        return {gitHubUrl:res.data}
      });
      return getUrl;
    }
    catch(error){
