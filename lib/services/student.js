...
module.exports = class StudentService extends Schmervice.Service {
    ...
    if (Stage) {
      if (FromDate && ToDate) {
        ...
        const promises = _.map(array,async(student)=>{
          student['studentDonor'] = Boolean(student['studentDonor']) ? student['studentDonor']['donor_id'] : student['studentDonor']
          student['campus'] = Boolean(student['campus']) ? student['campus']['campus'] :student['campus']
        for (var i in student['studentDonor']){
          const DonorName = await Donor.query().where({id:student['studentDonor'][i]})
  ...
  async getStudentById(id, txn = null) {
    ...
    console.log( await student[0]['studentDonor']);
    student[0]['studentDonor'] = await Boolean(student[0]['studentDonor'])? student[0]['studentDonor']['donor_id']:student[0]['studentDonor']
    student[0]['campus'] = await Boolean(student[0]['campus'])?student[0]['campus']['campus']:student[0]['campus']
    
    for (var i in student[0]['studentDonor']){
      const DonorName = await Donor.query().where({id:student[0]['studentDonor'][i]})
