...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async autoCallToJoinOnlineClassForFailedStudent(contacts) {
    const { exotelService } = this.server.services();
    const callPromises = [];
    _.each(contacts, (contact) => {
      callPromises.push(exotelService.autoCallingToFailedStudent(contact.Number))
    })
    
    const connectedAllCalls = Promise.all(callPromises);
    return connectedAllCalls;
  }
};
