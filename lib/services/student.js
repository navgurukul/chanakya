...
const { markRedFlag, joinDateValidation } = require("../helpers/redflags");
...
module.exports = class StudentService extends Schmervice.Service {
    ...
    if (contacts.length === 0) {
      ...
      const finalData = data.map((e) => {
        ...
        e.test_status = assessmentService.getEnrolmentKeyStatus(
          e.enrolmentKey[0].key
        );
        e.total_marks = e.enrolmentKey[0].total_marks;
  ...
  async checkForRedflag() {
    const { campusService, studentCampusService } = this.server.services();
    ...
    campusData.map(async (campus) => {
      ...
      data.map(async (student) => {
        ...
        //1.for the join date feild
        // console.log("Student: " + student.id);
        let redflag = "";
        const transitionsData = await studentService.getTransitionData(
          student.id
        );
        if (joinDateValidation(transitionsData) == false) {
          redflag += "Join date is invalid,";
        }
        //2.for the remaining feilds
        redflag += markRedFlag(student);
        redflag = redflag.replace(/,\s*$/, "");
        studentService.updateRedFlag(student.id, redflag);
      });
    });
  }
};
