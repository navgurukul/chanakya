...
module.exports = class StudentService extends Schmervice.Service {
 ...
 async markDuplicateOfSameStudent1(number, name, DOB) {
    ...
    let contacts = await Contact.query()
      ...
      .eager("student(filter)", {
        filter: function (builder) {
          builder.select("id", "stage", "name", "dob");
        },
      });
    contacts = contacts
      .filter((contact) => {
        if (
          ...
          contact.student.dob === null
        ) {
          return false;
        }
        if (
          contact.student.name.toLowerCase().split(" ").join("") ===
            name.toLowerCase().split(" ").join("") && contact.student.dob == String(new Date(DOB))
        ) {
          return true;
        }
      }).map(contact=>{
      ...
      if (contacts.length===0){
        ...
        const data = await Student.query().select("id","stage").whereIn("id",contacts)
        ...
        const finalData = data.map(e=>{
          ...
          e.test_status = assessmentService.getEnrolmentKeyStatus(e.enrolmentKey[0].key)
