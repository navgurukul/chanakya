...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async updateStudentDetails(student_id, details) {
    ...
    console.log(details);
    const updateName = await Student.query().patch(details).skipUndefined().where('id', student_id);
    return updateName;
  }
  async markDuplicateOfSameStudent1(number, name) {
    ...
    if (contacts.length === 0) {
      ...
      const finalData = data.map((e) => {
        ...
        e.test_status = assessmentService.getEnrolmentKeyStatus(e.enrolmentKey.slice(-1)[0].key);
  ...
  async updateEvaluationData(student_id, evaluation) {
    const { Student } = this.server.models();
    const updatedEvaluation = await Student.query().patch({ evaluation }).where('id', student_id);
  ...
  async deleteTransition(transitionId, txn) {
    const { StageTransition, Student, Feedback } = this.server.models();
    const transitions = await StageTransition.query(txn)
      .where('id', transitionId)
      .withGraphFetched({ feedbacks: true });
    if (transitions.length > 0) {
      const studentData = await Student.query().where('id', transitions[0].student_id);
    ...
    if (transitions[0] !== undefined && transitions[0].feedbacks !== null) {
      await Feedback.query(txn).where('id', transitions[0].feedbacks.id).delete();
    }
  ...
  async addOrUpdateName(student_id, name) {
    const { Student } = this.server.models();
    const updateName = await Student.query().patch({ name }).where('id', student_id);
    return updateName;
  }
  async findJobKabLagega(joindateAndLeaveDays, student, totalMilestone = 18) {
    ...
    let daysPassedInCampus = date.subtract(todayDate, joindate).toDays() - leaveDays;
    let kitneDinLagenge = (daysPassedInCampus / milestone.slice(1)) * totalMilestone;
    let kitneAurDin = kitneDinLagenge - (daysPassedInCampus + leaveDays);
    // console.log(kitneDinLagenge, daysPassedInCampus, kitneAurDin);
    let expectedDate = date.format(date.addDays(joindate, kitneDinLagenge), 'YYYY-MM-DD');
    return {
      daysPassedInCampus,
      kitneDinLagenge,
      kitneAurDin,
  ...
  async findStudentsByStageTransition(to_stage, WhereClause = {}, whereNotClause = {}, txn = null) {
  ...
  async createWithoutExam(Details = {}, txn = null) {
    ...
    if (stage == 'inJob') {
      details.transitions = [
        { to_stage: 'createdStudentWithoutExam' },
        { to_stage: 'finallyJoined' },
        { to_stage: stage },
      ];
    } else {
  ...
  async create(stage, incomingCall = null, Details = {}, txn = null) {
    ...
    details.contacts = [{ mobile, is_whatsapp: false, contact_type: 'primary' }];
    if (whatsapp) {
  ...
  swapEnumKeysWithValues(Details) {
    ...
    _.each(specialFields, (field) => {
      if (details[field]) {
        ...
        details.current_status = CONSTANTS.studentDetails[field][details.current_status];
      }
    });
    return details;
  }
  ...
  async studentsData(fromDate, toDate, txn = null) {
    ...
    if (FromDate && !ToDate) {
      var courseData = await Student.query(txn).where('created_at', '>=', FromDate);
      return courseData;
    }
    if (!FromDate && !ToDate) {
      var courseData = await Student.query(txn);
  ...
  async studentsDataForDashBoard(
    ...
    if (searchCampusName != '') {
      var searchCampus = await Campus.query()
        .select('id')
        .whereRaw(`LOWER(campus) LIKE ?`, [`%${searchCampusName.toLowerCase()}%`]);
    ...
    if (searchOwnerName != '') {
      var searchOwner = await Feedback.query()
        ...
        .whereRaw(`LOWER(to_assign) LIKE ?`, [`%${searchOwnerName.toLowerCase()}%`]);
      var ownerFilter = [];
      for (var n in searchOwner) {
        var owner = await Student.query().select('stage').where('id', searchOwner[n].id);
    ...
    if (searchByStatus.length > 0) {
      ...
      for (var n in searchStatus) {
        var status = await Student.query().select('stage').where('id', searchStatus[n].id);
        if (status[0] && status[0]['stage'].includes(searchStatus[n]['stage'])) {
    ...
    if (searchPartnerName != '') {
      var searchPartner = await Partner.query()
        .select('id')
        .whereRaw(`LOWER(name) LIKE ?`, [`%${searchPartnerName.toLowerCase()}%`]);
  ...
  async addStudentDetails(array) {
    ...
    const promises = _.map(array, async (student) => {
      ...
      student.campus =
        student.campus && student.campus.length > 0 ? student.campus[0].campus : null;
    ...
    _.each(danglingRecord, (details) => {
      ...
      if (stageTransition) {
        if (CONSTANTS.feedbackableStages.indexOf(details.stage) > -1) {
          const diffTime = Math.abs(new Date() - new Date(stageTransition.created_at));
  ...
  async addOrUpdateEmail(email, student_id) {
    const { Student } = this.server.models();
    const updateEmail = await Student.query().patch({ email }).where('id', student_id);
    return updateEmail;
  }
  async updateRedFlag(student_id, redflag) {
    ...
    const { Student } = this.server.models();
    const updatedFlag = await Student.query().patch({ redflag: redflag }).where('id', student_id);
    return updatedFlag;
  }
  ...
  async markDuplicateOfSameStudent(number, name, enrolmentKey, assessmentService) {
    const { Contact, Student } = this.server.models();
  ...
  async getJobDescription() {
    const { Student } = this.server.models();
    const data = await Student.query()
      .select('id as student_id', 'name', 'gender', 'email', 'qualification')
