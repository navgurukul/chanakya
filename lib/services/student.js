...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async dashboardRecordDistribution(txn = null) {
    const { Student } = this.server.models();
    const studentStage = CONSTANTS.studentStages;
    const dashboardRecord = {};

    const totalRecord = await Student.query(txn).count('id as count');
    dashboardRecord['Total Record'] = totalRecord[0].count;

    await Promise.all(_.map(studentStage, async (stage) => {
      const allCount = {};
      const Stage = stage.replace(stage.charAt(0), stage.charAt(0).toUpperCase()).match(/[A-Z][a-z]+|[0-9]+/g).join(' ');
      const dangling_record = await Student.query(txn).eager({
        danglingtranstions: true,
      });
      const dangling = [];
      _.each(dangling_record, (details) => {
        if (stage === details.stage) {
          const [date] = details.danglingtranstions;
          if (date) {
            const diffTime = Math.abs(new Date() - new Date(date.createdAt));
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
            if (diffDays > 3) {
              dangling.push({numberOfDays: diffDays, name: details.name, ActualStage: stage });
            }
          }
        }
      });
      const total = await Student.query(txn).where('stage', stage).count('stage as count');
      const femaleCount = await Student.query(txn).where('stage', stage)
        .andWhere('gender', 1).count('stage as count');
      const maleCount = await Student.query(txn).where('stage', stage)
        .andWhere('gender', 2).count('stage as count');
      allCount.Total = total[0].count;
      allCount.Female = femaleCount[0].count;
      allCount.Male = maleCount[0].count;
      allCount['Dangling Total'] = dangling;
      dashboardRecord[Stage] = allCount;
    }));

    return dashboardRecord;
  }

  async ownerReport(txn = null) {
    const { Student } = this.server.models();
    const students = await Student.query(txn).eager({ feedback: true });
    const details = [];
    _.each(students, (student) => {
      if(student.feedback.length) {
