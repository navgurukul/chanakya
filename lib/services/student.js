...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async findJobKabLagega(joindate, milestone, totalMilestone = 18) {
    ...
    let kitneAurDin = kitneDinLagenge - daysPassed;
    // console.log(kitneDinLagenge, daysPassed, kitneAurDin);
    let expectedDate = date.format(
      date.addDays(joindate, kitneDinLagenge),
      "YYYY-MM-DD"
    );
    return { daysPassed, kitneDinLagenge, kitneAurDin, expectedDate };
  }
  ...
  async getStudentById(id, txn = null) {
    ...
    const date = await feedbackService.joinedCampusDate(studentId);
