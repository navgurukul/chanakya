...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async studentsDataForDashBoard(
    ...
    if (testMode) {
      let l = [];
      let searchTestMode = await EnrolmentKey.query().where({ "type_of_test": testMode }).page(page, limit)
      for (const i of searchTestMode.results){
        for (const j in i){
          if (!(l.includes(i.student_id))){
            l.push(i.student_id)
          }
        }
      }
      ...
      let student;
      let dic = [];
      for (let studentId of l){
          student = await Student.query(txn).where("id", studentId).withGraphFetched({
          partner: true,
          contacts: true,
          enrolmentKey: true,
          partnerAssessment: true,
        ...
        dic.push((await this.addStudentDetails(student))[0])
      }
      return this.commonFunc(dic)
    }
