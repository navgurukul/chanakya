...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async insertOrUpdateJobDescription(data){
    const {  studentJobDetails } = this.server.models();
    const documents = await studentJobDetails.query().where('student_id',data.student_id)
    let changes;
    if (documents.length===0){
        changes = await studentJobDetails.query().insert({...data})
    }else{
        changes = await studentJobDetails.query().update({...data}).where('student_id',data.student_id)
    }
    return changes
  }
  ...
  async getJobDescription(){
    const { Student } = this.server.models();
    const data = await Student.query().select("id as student_id","name","gender")
