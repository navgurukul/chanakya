...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async studentsDataForDashBoard(
    ...
    if (Stage) {
      if (FromDate && ToDate) {
        ...
        _.map(array,(student)=>{
          student['donor'] = Boolean(student['donor']) ? student['donor']['donor'] : student['donor']
  ...
  async getStudentById(id, txn = null) {
    ...
    student[0]['donor'] = await Boolean(student[0]['donor'])? student[0]['donor']['donor']:student[0]['donor']
