...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async leaveDaysCalculate(from, to) {
    const leave = date.subtract(to, from).toDays();
    return leave;
  }
  ...
  async findJobKabLagega(joindateAndLeaveDays, student, totalMilestone = 18) {
    const leaveDays = joindateAndLeaveDays[1];
    const joindate = joindateAndLeaveDays[0];
    ...
    const daysPassedInCampus = date.subtract(todayDate, joindate).toDays() - leaveDays;
    const kitneDinLagenge = (daysPassedInCampus / milestone.slice(1)) * totalMilestone;
    const kitneAurDin = kitneDinLagenge - (daysPassedInCampus + leaveDays);
    // console.log(kitneDinLagenge, daysPassedInCampus, kitneAurDin);
    const expectedDate = date.format(
  ...
  async studentStatus(mobileNumber, txn = null) {
    ...
    _.each(contact, (studentInfo) => {
      ...
      if (updatedStage.to_stage !== 'requestCallback') {
        ...
        studentStatus.linkForEnglishTest = updatedStage.to_stage === 'pendingEnglishInterview' ? null : null;
        studentStatus.linkForOnlineTest = updatedStage.to_stage === 'enrolmentKeyGenerated'
      ...
      courseData.results = courseData.results.filter((data) => {
        if (data.studentDonor != null) {
          return data.studentDonor
            .map((data) => data.donor.includes(searchDonorName))
            .includes(true);
        }
        return false;
      });
  ...
  async studentsDataForDashBoard(
    ...
    const {
      Student, Contact, Partner, Feedback, Campus, studentCampus,
    } = this.server.models();
    ...
    if (SearchNumber != '') {
      const numberSearch = await Contact.query()
        .select('student_id as id')
        .where('mobile', 'like', `%${SearchNumber}%`);
      for (const n in numberSearch) {
    ...
    if (searchOwnerName != '') {
      ...
      for (var n in searchOwner) {
        const owner = await Student.query()
          .select('stage')
    ...
    if (searchByStatus.length > 0) {
      const searchStatus = await Feedback.query()
        .select('student_id as id', 'student_stage as stage')
    ...
    if (searchPartnerName != '') {
      const searchPartner = await Partner.query()
        .select('id')
        .whereRaw('LOWER(name) LIKE ?', [
          `%${searchPartnerName.toLowerCase()}%`,
        ]);
      for (const n in searchPartner) {
  ...
  async addStudentDetails(array) {
    ...
    const promises = _.map(array, async (student) => {
      ...
      student.campus = student.campus && student.campus.length > 0
  ...
  async markDuplicateOfSameStudent(
    ...
    let contacts = await Contact.query()
      .select('id')
      .where('mobile', number)
      .eager('student(filter)', {
