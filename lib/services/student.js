...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async commonFunc(
    ...
    courseData.results = courseData.results.filter((data) => {
      ...
      return data.campus.includes(searchCampusName);
    });
    courseData.results = courseData.results.filter((data) => {
      ...
      return data.partnerName.includes(searchPartnerName);
    });
    courseData.results = courseData.results.filter((data) => {
    ...
    courseData.results = courseData.results.filter((data) => {
      if (data.feedback != []) {
        return data.feedback
          .map((data) => {
            if (data.to_assign != null) {
              return data.to_assign.includes(searchOwnerName);
            } else {
              if (searchOwnerName == "") {
                return true;
              } else {
    ...
    courseData.results = courseData.results.filter((data) => {
      if (data.contacts !== null) {
        return data.contacts
          .map((Data) => {
            return Data.mobile.includes(SearchNumber);
          })
          .includes(true);
      }
    });
    ...
    if (Stage) {
      if (FromDate && ToDate) {
        const courseData = await Student.query(txn)
          ...
          .andWhere("created_at", ">=", FromDate)
          .andWhere("created_at", "<=", ToDate)
          .andWhere("name", "like", `%${searchName}%`)
          .limit(limit)
          .offset(page)
          .eager(eagerRelation);
        const array = await this.getStudentDetalisByStage(courseData);

        const studentArray = this.addStudentDetails(array);
        return studentArray;
      }
      if (FromDate && !ToDate) {
        const courseData = await Student.query(txn)
      ...
      if (!FromDate && !ToDate) {
        const courseData = await Student.query(txn)
          ...
          .orderBy("last_updated", "desc")
          .whereIn("gender", gender)
          .whereRaw(`LOWER(name) LIKE ?`, [`%${searchName.toLowerCase()}%`])
          .page(page, limit)
          .eager(eagerRelation);
        const array = await this.getStudentDetalisByStage(courseData);
        const studentArray = this.addStudentDetails(array);
