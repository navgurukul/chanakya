...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async sendEnrolmentKey(Options) {
    ...
    let student; let key;
    if (options.mobile) {
      student = await this.create('enrolmentKeyGenerated', options.fromHelpline ? 'getEnrolmentKey' : null, { mobile: options.mobile, partner_id: partner_id });
  ...
  studentStageNotChangeSince(student) {
    if (student.transitions.length > 0) {
      const lastStageChangeAt = _.max(_.map(student.transitions,
        (transitions) => transitions.created_at));
  ...
  swapEnumKeysWithValues(Details) {
    ...
    const specialFields = ['gender', 'qualification', 'currentStatus', 'caste', 'religon', 'school_medium'];
  ...
  async studentStatus(mobileNumber, txn = null) {
    ...
    _.each(contact, (studentInfo) => {
      ...
      if (updatedStage.to_stage !== 'requestCallback') {
        ...
        studentStatus.linkForEnglishTest = updatedStage.to_stage === 'pendingEnglishInterview'
          ? null : null;
  ...
  async addAndUpdateContact(details, student_id, txn=null) { // Add or Update contact Number.
    const { Contact } = this.server.models();
    ...
    if(addOrUpdate == 'addContact') {
      ...
      const updateContact = await Contact.query(txn).patch(details).where('student_id', student_id).where("contact_type", details.contact_type)
  ...
  async getContacts(student_id) { // getting student contact for extra details to showing on dashboard.
    const { Contact } = this.server.models();
