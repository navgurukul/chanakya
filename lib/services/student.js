...
module.exports = class StudentService extends Schmervice.Service {
  ...
  async studentsDataFromDb (typeOfStage, txn = null){
    const { Student } = this.server.models();
    const studentTypeOfData = typeOfStage;
    
    if (studentTypeOfData === 'requestCallback') {
      const rqcData = await Student.query(txn).where({ stage: studentTypeOfData});
      return rqcData;
    } else {
      const otherData = await Student.query(txn).whereNot({ stage: 'requestCallback'});
      return otherData;
    }

  }
  
  async studentsDataForDashBoard(typeOfData, fromDate, toDate, lastDate, txn = null){

    const FromDate = fromDate; const ToDate = toDate; const LastDate = lastDate;
    ...
    if (studentTypeOfData === 'requestCallback') {
      ...
      if (LastDate && !FromDate && !ToDate) {
        const rqcData = await Student.query(txn)
        .where('stage', studentTypeOfData)
        .andWhere('createdAt', '>=', LastDate);
      return rqcData;
      }
    } else {
      if (FromDate && ToDate){
