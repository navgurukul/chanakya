...
module.exports = class MetricsService extends Schmervice.Service {
  ...
  async getStageTransitionMetrics(fromStage, toStage, startDate, endDate) {
    ...
    let transitionTimes = _.map(transitions, t => t.transitionTime);
    let transTimesLen = transitionTimes.length;
    let transTimesHalfLen = Math.floor(transTimesLen/2)
    let median = transTimesLen % 2 === 0 ? (transitionTimes[transTimesHalfLen]) : (
