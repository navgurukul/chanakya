...
module.exports = class MetricsService extends Schmervice.Service {
  ...
  _newPartnerStudentsBaseQuery(date) {
    ...
    const baseQuery = this._newStudentsBaseQuery(date);

    return baseQuery.clone()
      .innerJoin('enrolment_keys')
  ...
  async recordAllMetrics(date, recordNonDate = false) {
    ...
    const newPartnerStudentsQuery = this._newPartnerStudentsBaseQuery(date);
    ...
    const newHelplineStudentsQuery = this._newHelplineStudentsBaseQuery(date);
