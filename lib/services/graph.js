...
module.exports = class graphService extends Schmervice.Service {
  ...
  async graph(data) {
    const graphDataReal = [
      { name: "Time  < 9 Months", value: 0 },
      { name: "Time  >=9 months and <12 months", value: 0 },
      { name: "Time  >=12 months and <15 months", value: 0 },
      { name: "Time  >=15 months and <18 months", value: 0 },
      { name: "Time   >=18 months", value: 0 },
    ];
    ...
    graphDataReal[4].value = await this.percentage(
      graphDataReal[4].value,
      totalLength,
      jobKabLagegaNull
    );
    graphDataReal[3].value = await this.percentage(
      graphDataReal[3].value,
      totalLength,
      jobKabLagegaNull
    );
    graphDataReal[2].value = await this.percentage(
      graphDataReal[2].value,
      totalLength,
      jobKabLagegaNull
    );
    graphDataReal[1].value = await this.percentage(
      graphDataReal[1].value,
      totalLength,
      jobKabLagegaNull
    );
    graphDataReal[0].value = await this.percentage(
