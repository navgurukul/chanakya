...
module.exports = class sendingSmsToStudents extends Schmervice.Service {
    async prepareSmsTemplate(id) {
        ...
        if (studentDetails[0].stage === 'pendingEnglishInterview') {
            ...
            templateId = CONSTANTS.DLT.auth.templatePassId;
        } else if (studentDetails[0].stage === 'testFailed') {
            templateName = 'online_test_fail.txt';
            templateId = CONSTANTS.DLT.auth.templateFailId;
        ...
        if (templateId) {
            return {formattedData, templateId}
        }
    ...
    async sendSmsToStudents(formattedData, id, templateId) {
        ...
        try {
            ...
            const params = {
                ...
                // EntityId: CONSTANTS.DLT.auth.entityId, 
                // TemplateId: templateId, 
                // SenderId: CONSTANTS.DLT.auth.senderId, 
                MessageAttributes: {
                    // 'SenderId': {
                    //   DataType: 'String',
                    //   StringValue: CONSTANTS.DLT.auth.senderId 
                    // },
                    // 'EntityId': {
                    //     DataType: 'String',
                    //     StringValue: CONSTANTS.DLT.auth.entityId 
                    //   },
                    //   'TemplateId': {
                    //     DataType: 'String',
