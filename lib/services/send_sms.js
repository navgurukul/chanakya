...
module.exports = class sendingSmsToStudents extends Schmervice.Service {
  async prepareSmsTemplate(id) {
    ...
    if (studentDetails[0].stage === 'pendingEnglishInterview') {
      ...
      studentDetails[0].stage !== '' &&
      studentDetails[0].stage !== 'offerLetterSent' &&
      studentDetails[0].stage !== 'testFailed' &&
      studentDetails[0].stage !== 'pendingEnglishInterview'
    ) {
    ...
    console.log('studentDetails in sms', studentDetails);
    ...
    const replacedSchool = replacedStage.replace('<School Name>', studentDetails[0].school); // How to get school as it's considering email but not school name
