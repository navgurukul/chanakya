 ...
 * } reports
 */
const sendPartnersReports = (data, recivingEmails) => {
  const source = fs.readFileSync(
    CONSTANTS.gSheet.partnersReports.partnerTemplate,
    "utf-8"
  );

  const htmlString = handleBar.compile(source)(data);
  const { subject } = CONSTANTS.gSheet.emailReport;
  return SESEmail(recivingEmails, htmlString, [], subject, true);
};
