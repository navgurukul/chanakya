...
smsTemplates.closeBoysAdmission = `Hey, thanks so much for applying. Admissions are only on open for girls, women and transgender people. It would be great if you can apply again after 3 months.\n

Apply करने के लिए धन्यवाद। नवगुरुकुल अभी सिर्फ लड़कियों, औरतो और transgender के लिए खुला है।  हम आपको encourage करते है की आप 3 महीने के बाद aaply करें।`
...
smsTemplates.deadlineMessage = `Hello, {{=it.user.to_assign}} aap ko {{=it.user.who_assign}} ne' {{=it.student.name}}' ka {{=it.student.stage}} ka interview assign kiya tha.

6 hours bache hai deadline khatam hone mei, please time se kar do!`;

smsTemplates.deadlineEndMessage = `Hello, {{=it.user.to_assign}} aap ko {{=it.user.who_assign}} ne '{{=it.student.name}}' ka {{=it.student.stage}} ka interview assign kiya tha.

Aur uski deadline khatam ho gayi hai. Please jaldi se kar lo yaar!

Thank You!`;

smsTemplates.assignWork = `Hello, {{=it.user.to_assign}} aap ko {{=it.user.who_assign}} ne '{{=it.student.name}}' ka {{=it.student.stage}} ka interview assign kiya hai.

Please jaldi se kar lo yaar!`;

smsTemplates.informToCompleteTheTest = `Hello, {{=it.student.name}} apne NavGurukul ki scholarship ke liye apply kiya hai aur abhi tak apne test complete nahi kiya hai. 

Aap test complete karne ke liye es link ka use kare http://join.navgurukul.org/k/{{=it.key}} test pass hone ke baad jald hi hamari team contact karengi 
...
module.exports = {
  ...
  closeBoysAdmission: dot.template(smsTemplates.closeBoysAdmission),
  informToCompleteTheTest: dot.template(smsTemplates.informToCompleteTheTest)
};
