...
module.exports = class ChanakyaGSheetSync {
    ...
    async addPendingEnrolmentKeys(existingKeys) {
        ...
        let rows = _.map(enrolmentKeys, (key) => {
            ...
            let row = {
                ...
                studentCreatedAt: key.student.createdAt
            }
    ...
    async startSync() {
        ...
        await this.addPendingEnrolmentKeys(entries.enrolmentKeys);
        await this.addPendingIncomingCalls(entries.incomingCallIds);

        // update the existing incoming call and enrolment key rows
        await this.updateExistingIncomingCallEntries(entries.incomingCallIds);
        await this.updateExistingEnrolmentKeyEntries(entries.enrolmentKeys);
    }
}
