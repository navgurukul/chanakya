...
module.exports = class ChanakyaGSheetSync {
    ...
    _hasStudentStageChanged(key, currentStageOnSheet) {
        return key.student.stage !== currentStageOnSheet;
    }
    ...
    shouldRecordStudentTranisition(key, currentStageOnSheet) {
        console.log("Updating the students record on the platform from gSheet.")
        // if any stage would be added in the sheet in the future.
        if (currentStageOnSheet === undefined) {
            return false
        }
        if (!this._isStageValid(currentStageOnSheet)) {
            throw `${currentStageOnSheet} stage is not added in studentStages enums in constants.js.`;
        }
        
        // update student stage with the stage on the gSheet & create a stage transition record.
        if (this._hasStudentStageChanged(key, currentStageOnSheet)) {
            return true;
        } else {
            return false;
        }
    ...
    async syncPlatformWithGSheet(existingKeys) {
        ...
        _.each(existingKeys, async (key) => {
            ...
            if (this.shouldRecordStudentTranisition(dbKey, key.student.actualStage)) {
