...
module.exports = class ChanakyaGSheetSync {
    ...
    async addPendingEnrolmentKeys(existingKeys) {
        ...
        let rows = _.map(enrolmentKeys, async (key) => {
            let row = {
                taskType: "test",
            ...
            // Sync paperSet into Gsheet.
            if (key.questionSetId !== null) {
                let paperSet = await this.partnerService.paperSet(key.questionSetId);
                if (paperSet){
        ...
        let new_rows = await Promise.all(rows)
        this.reports.keysAdded += new_rows.length
        await this.addRows(new_rows);
    }
