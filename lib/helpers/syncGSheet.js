...
module.exports = class ChanakyaGSheetSync {
    ...
    async addPendingEnrolmentKeys(existingKeys) {
        ...
        let rows = _.map(enrolmentKeys, (key) => {
            ...
            if (key.questionSet !== null){
                let testVersion = key.questionSet.testVersion.name;
                let totalMarks = key.totalMarks;
                let studentStage = key.student.stage;
                row["testVersion"] = testVersion
                
                // According to the cut off marks change Actual Stage
                if (key.student.gender == 2){ // cut off marks for Female
                    if ((studentStage == "completedTestWithDetails" || studentStage == "testPassed") && totalMarks >= CONSTANTS.testCutoffMale[testVersion]) {
                        row["stage"] = "testPassed"
                        row["Actual Stage"] = "Test Passed"
                    }else if ( studentStage == "completedTestWithDetails" || studentStage == "testFailed" ){
                        row["stage"] = "testFailed"
                        row["Actual Stage"] = "Test Failed"
                    }    
                
                }else if (key.student.gender == 1){ // Cut off marks for Male
                    if ( (studentStage == "completedTestWithDetails" || studentStage == "testPassed") && totalMarks >= CONSTANTS.testCutoffFemale[testVersion]) {
                        row["stage"] = "testPassed"
                        row["Actual Stage"] = "Test Passed"
                    }else if (studentStage == "completedTestWithDetails" || studentStage == "testFailed"){
                        row["stage"] = "testFailed"
    ...
    async updateExistingEnrolmentKeyEntries(existingKeys) {
        ...
        _.each(existingKeys, (key) => {
            let dbKey = enrolmentKeys[key.key];
            let actualStage = key.student.actualStage
            if(!dbKey) {
                return
            }
            // record students 'Stage Transition History' in gSheet.
            let transitions = dbKey.student.transitions;
            let stageTransitionHistory = "";
            _.each(transitions, (trnsition) => {
                let date = moment(trnsition.createdAt).format('LL');
                stageTransitionHistory = stageTransitionHistory + date + ': ' + trnsition.toStage + "\n"
            })
            ...
            // auto change rqcStatus and Actual Stage.
            if (dbKey.student.stage == "testPassed") {
                row["Actual Stage"] = "Pending Callback for Query"
                row["stage"] = "pendingCallbackForQuery"
                row["rqcStatus"] = "Pending"
            }
            // assing equal ammount of work to all the tribe
            if (!actualStage.length) {
                actualStage = false;
            }
            
            if ( (actualStage != "testFailed") && actualStage) {  
                row["rqcOwnerTribe"] = CONSTANTS.rqcOwnerTribe[tribCount];
                if (tribCount < 6){
                    tribCount +=1
                }else{
                    tribCount = 0
    ...
    async updateExistingIncomingCallEntries(existingCallIds) {
        ...
        _.each(existingCallIds, (id) => {
            ...
            // Record incomming call students "Stage transition History" in gSheet.
            let transitions = dbCall.contact.student.transitions;
            var stageTransitionHistory = "";
            _.each(transitions, (trnsition) => {
                let date = moment(trnsition.createdAt).format('LL');
                stageTransitionHistory = stageTransitionHistory + date + ': ' + trnsition.toStage + "\n"
            })
    ...
    async updateExistingEnrolmentKeysStageTransitionsHistory(existingKeys) {

        console.log("Updating students stage transitions history using existing enrolment keys.");
    ...
    async updateExistingIncomingCallStagetTransitionHistory(existingCallIds) {

        console.log("Updating existing incoming call Students stage transition history.");
    ...
    async updateSheetForStageTransitionHistory() {
        // get the ids of existing incoming call ids and enrolment keys in the spreadsheet
        let entries = await this.getExistingEntries()

        // update incoming call types studetns stage transitions history.
        this.updateExistingIncomingCallStagetTransitionHistory(entries.incomingCallIds)
        .then(() => {
            // update students stage transitions history using existing students enrolment keys.
            return this.updateExistingEnrolmentKeysStageTransitionsHistory(entries.enrolmentKeys)
        })
    }
    async syncFromSheet() {
        // get the ids of existing incoming call ids and enrolment keys in the spreadsheet
    ...
    async addDataToSheet() {
        // get the ids of existing incoming call ids and enrolment keys in the spreadsheet
        let entries = await this.getExistingEntries();
        // Reupdating Gsheet from database onces databse is updated with students actual stage and pending enrolment_keys and student's details.
