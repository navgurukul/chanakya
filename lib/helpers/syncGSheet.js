...
internals.getColumnNameFromPosition = (Num) => {
  let num = Num;
  let ret = ''; let a = 1; let b = 26;
  for (ret, a, b; (num -= a) >= 0; a = b, b *= 26) {
...
module.exports = class ChanakyaGSheetSync {
  ...
  async syncPlatformEnrolmentKeysWithGSheetforStudentStatus(existingKeys, mobile) {
    ...
    _.each(existingKeys, async (key) => {
      ...
      // sent student status.
      if (contact.some((contacts) => contacts.mobile === mobile) && key.student.actualStage !== 'pendingEnglishInterview') {
  ...
  async addPendingEnrolmentKeys(ExistingKeys) {
    ...
    const rows = _.map(enrolmentKeys, (key) => {
      ...
      // add actual stage
      row = this.addActualStageToRow(row);
  ...
  async studentStatus(mobile) {
    // get the ids of existing incoming call ids and enrolment keys in the
    // spreadsheet for students status.
