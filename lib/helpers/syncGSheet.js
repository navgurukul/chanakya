...
module.exports = class ChanakyaGSheetSync {
    ...
    async addPendingEnrolmentKeys(existingKeys) {
        ...
        let rows = _.map(enrolmentKeys, (key) => {
            ...
            if (key.questionSet !== null){
                ...
                // According to the cut off marks desided the students result and changed their Actual Stage as well as stage.
                if (key.student.gender == CONSTANTS.studentDetails.gender.female){ // cut off marks for Female
                    if ( this.testPass(studentStage) && totalMarks >= CONSTANTS.testCutoffMale[testVersion]) {
                        row["stage"] = "testPassed"
                        row["Actual Stage"] = "Test Passed"
                    }else if (this.testFailed(studentStage)){
                        row["stage"] = "testFailed"
                        row["Actual Stage"] = "Test Failed"
                    }

                }else if (key.student.gender == CONSTANTS.studentDetails.gender.male){ // Cut off marks for Male
                    if (this.testPass(studentStage) && totalMarks >= CONSTANTS.testCutoffFemale[testVersion]) {
                        row["stage"] = "testPassed"
                        row["Actual Stage"] = "Test Passed"
                    }else if (this.testFailed(studentStage)){
                        row["stage"] = "testFailed"
    ...
    async updateExistingEnrolmentKeyEntries(existingKeys) {
        ...
        _.each(existingKeys, (key) => {
            ...
            // assigned work for tribe when students stage is testPass auto change rqcStatus and Actual Stage as you mention in git hub.
            ...
            // some time what happend the actualStage is blank at that time it's return empty string and according to my logic we only assigned work for
            // testPassed student. for that purpose I have changed actualstage as false.
            if (!actualStage.length) {
                actualStage = false;
            }
            // finally assigned equal ammount of work for all tribes onces student actual stage is testPassed
            if ( (actualStage != "testFailed") && actualStage) {  
                row["rqcOwnerTribe"] = CONSTANTS.rqcOwnerTribe[tribeCount];
                if (tribeCount < 6){
                    tribeCount +=1
                }else{
                    tribeCount = 0
