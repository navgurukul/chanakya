...
// we can catch releative error and sent through email
  (_.keys(internals.stages).includes(stage) ? internals.stages[stage] : stage);
...
module.exports = class ChanakyaGSheetSync {
  ...
  async syncPlatformEnrolmentKeysWithGSheetforStudentStatus(existingKeys, mobile) {
    const enrolmentKeys = await this.getEnrolmentKeysObject(existingKeys);
    const studentDetails = [];

    _.each(existingKeys, async (key) => {
      const dbKey = enrolmentKeys[key.key];
      const contact = dbKey.student.contacts;
      const studentStatus = {};
      if (!dbKey) {
        return;
      }
      // sent student status.
      if (contact.some((contact) => contact.mobile == mobile) && key.student.actualStage != 'pendingEnglishInterview') {
        studentStatus.CurrentStage = key.student.actualStage;
        studentStatus.LastUpdate = dbKey.student.stage;
        studentStatus.Name = dbKey.student.name;
        studentStatus.City = dbKey.student.city;
        studentStatus.State = CONSTANTS.studentDetails.states[dbKey.student.state];
        studentStatus.Marks = dbKey.totalMarks;
        studentStatus.Gender = _.invert(CONSTANTS.studentDetails.gender)[dbKey.student.gender];
        studentStatus.linkForEnglishTest = "You have already taken the English test or You haven't given an online test yet or you have failed the online test";
        studentStatus.Transitions = dbKey.student.transitions;
        studentDetails.push(studentStatus);
      }
      // send student status with english test link if student not given test.
      if (contact.some((contact) => contact.mobile == mobile) && key.student.actualStage == 'pendingEnglishInterview') {
        studentStatus.CurrentStage = key.student.actualStage;
        studentStatus.LastUpdate = dbKey.student.stage;
        studentStatus.Name = dbKey.student.name;
        ...
        studentStatus.Marks = dbKey.totalMarks;
        studentStatus.Gender = _.invert(CONSTANTS.studentDetails.gender)[dbKey.student.gender];
        studentStatus.linkForEnglishTest = `http://join.navgurukul.org/k/${dbKey.key}`;
        studentStatus.Transitions = dbKey.student.transitions;
        studentDetails.push(studentStatus);
      }
    });
  ...
  async addPendingEnrolmentKeys(existingKeys) {
    ...
    const rows = _.map(enrolmentKeys, (key) => {
      ...
      // add actual stage
      row = this._addActualStageToRow(row);
  ...
  async getEnrolmentKeysObject(existingKeys) {
    ...
    let enrolmentKeys = await this.studentService.findEnrolmentKeyByKey(_.map(existingKeys, (key) => key.key));
  ...
  async getIncomingCallsObject(existingCallIds) {
    ...
    let incomingCalls = await this.studentService.findIncomingCallById(_.map(existingCallIds, (id) => id.id));
  ...
  async studentStatus(mobile) {
    // get the ids of existing incoming call ids and enrolment keys in the spreadsheet for students status.
    const entries = await this.getExistingEntries();

    // syncing Gsheet and get students's status.
    return this.syncPlatformEnrolmentKeysWithGSheetforStudentStatus(entries.enrolmentKeys, mobile);
  }
  ...
  async addDataToSheet() {
    ...
    // })
      .then(() => this.updateExistingIncomingCallEntries(entries.incomingCallIds)).then(() => this.addPendingIncomingCalls(entries.incomingCallIds)).then(() => this.updateExistingEnrolmentKeyEntries(entries.enrolmentKeys))
