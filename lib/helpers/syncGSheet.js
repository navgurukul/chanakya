...
module.exports = class ChanakyaGSheetSync {
    ...
    async _addStudentDetailsToRow(row, student) {
        const stageNotChangeSince = await this.studentService.studentStageNotChangeSince(student);
        row = _.extend(row, {
            name: student.name,
        ...
        // add students type of test in gsheet
        row.typeOfTest = student.typeOfTest
    ...
    async addPendingEnrolmentKeys(existingKeys) {
        ...
        let rows = _.map(enrolmentKeys, (key) => {
            ...
            row = await this._addStudentDetailsToRow(row, key.student);
    ...
    async addPendingIncomingCalls(existingCallIds) {
        ...
        let rows = _.map(incomingCalls, (incomingCall) => {
            ...
            row = await this._addStudentDetailsToRow(row, incomingCall.contact.student);
            return row;
        });
    ...
    async updateExistingEnrolmentKeyEntries(existingKeys) {
        ...
        _.each(existingKeys, (key) => {
            ...
            row = await this._addStudentDetailsToRow(row, dbKey.student);
    ...
    async updateExistingIncomingCallEntries(existingCallIds) {
        ...
        _.each(existingCallIds, (id) => {
            ...
            row = await this._addStudentDetailsToRow(row, dbCall.contact.student);
