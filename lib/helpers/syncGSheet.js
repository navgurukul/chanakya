...
module.exports = class ChanakyaGSheetSync {
    ...
    async updateExistingEnrolmentKeyEntries(existingKeys) {
        ...
        let ExistingEntry = await this.getExistingEntries()
        // console.log(ExistingEntry.enrolmentKeys[0].student)
        let enrolmentKeys = await this.getEnrolmentKeysObject(existingKeys)

        // build a list of updates that need to be done
        let maxColNum = _.keys(this.headers).length;
        _.each(existingKeys, (key,index) => {
            let dbKey = enrolmentKeys[key.key];
            let actualStage = ExistingEntry.enrolmentKeys[index].student.actualStage;
            if(!dbKey) {
                return
            }
            ...
            if (dbKey.questionSet !== null){
                let testVersion = dbKey.questionSet.testVersion.name;
                let totalMarks = dbKey.totalMarks;
                row["testVersion"] = testVersion
                
                // According to the cut off marks change Actual Stage
                if (dbKey.student.gender == 2){ // cut off marks for Female
                    if (dbKey.student.stage == "completedTestWithDetails" && totalMarks >= CONSTANTS.testCutoffMale[testVersion]) {
                        row["stage"] = "testPassed"
                        row["Actual Stage"] = "Test Passed"
                    }else if (dbKey.student.stage == "completedTestWithDetails" && totalMarks < CONSTANTS.testCutoffMale[testVersion]){
                        row["stage"] = "testFailed"
                        row["Actual Stage"] = "Test Failed"
                    }    
                
                }else if (dbKey.student.gender == 1){ // Cut off marks for Male
                    if (dbKey.student.stage == "completedTestWithDetails" && totalMarks >= CONSTANTS.testCutoffFemale[testVersion]) {
                        row["stage"] = "testPassed"
                        row["Actual Stage"] = "Test Passed"
                    }else if (dbKey.student.stage == "completedTestWithDetails" && totalMarks < CONSTANTS.testCutoffFemale[testVersion]){
                        row["stage"] = "testFailed"
                        row["Actual Stage"] = "Test Failed"
                    }
                }
            ...
            // If student stage is already "testPassed" is there and Actual Stage is null then change Actual Stage as "Test Passed" 
            if( dbKey.student.stage == "testPassed" && !actualStage && actualStage != "Test Passed"){
                row["Actual Stage"] = "Test Passed"
            }
