...
module.exports = class MDTokensToMD {
    ...
    render() {
        _.map(this.tokens, (token, index) => {
            let renderMethod = this.renderer[token.type];
            if (!renderMethod) {
    ...
    updateOptionsTable (tokenIndex, options) {
        let cells = this.tokens[tokenIndex].cells;
        let newCells = _.map(options, (option, index) => {
            return [index+1, cells[index][1], option.id]
        });
