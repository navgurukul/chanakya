...
class QuestionSeeder {
    ...
    static questionTypeFlag() {
        if (process.argv.indexOf("--questionType") > -1 ){
            let questionTpye = process.argv[process.argv.indexOf("--questionType") + 1]
            if (questionTpye == 'mcq'){
                return "mcqTest"
            }else{
                return "english"
    ...
    static getQuestionDirValue() {
        if (process.argv.indexOf("--questionsDir") > -1 && process.argv.indexOf("--questionType")){
            let questionsDir = process.argv[process.argv.indexOf("--questionsDir") + 1] + '/' + process.argv[process.argv.indexOf("--questionType") + 1];
            if (!questionsDir) {
    ...
    static passageAndQuestionTypeFlag() {
        if (process.argv.indexOf("--addQuestionsAndPassages") > -1) {
            return true
        }else{
            return false
        }
...
if (!module.parent) {
    ...
    let questionTypeFlag = QuestionSeeder.questionTypeFlag()
    let addAllQuestionsAndPassages = QuestionSeeder.passageAndQuestionTypeFlag()
    ...
    // both the question and passage addition and update MD flag cannot be active at the same time
    if (addAllQuestionsAndPassages && updateMdFlag) {
        QuestionSeeder.showErrorAndExit("--addAllQuestions & --updateMD cannot be active at once.");
    }
    ...
    if (questionTypeFlag == "mcqTest"){
        ...
        let config = QuestionSeeder.verifyEnglishConfigFile(questionsDir);
         // start the parsing for English test.
         let seeder = new englishTest(config, questionsDir, updateMdFlag, addAllQuestionsAndPassages);
         seeder.startParsing()
         .then(() => {
             console.log(`\nYou've successfully added English test passages and their Questions in DB.`)
         })
    }
