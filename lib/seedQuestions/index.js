...
class QuestionSeeder {
    ...
    constructor(config, updateMDFiles, addAllQuestions, noUpdateToConfigFlag, ignoreConfigBucketsAndChoices) {
        this.config = config;
        this.questionFiles = [];
        this.regexs = {
...
if (!module.parent) {
    ...
    if ( noUpdateToConfigFlag && !readlineSync.keyInYN("Are you sure you want to update the config.json file") ) {
    ...
    // start the parsing
    let seeder = new QuestionSeeder(config, updateMdFlag, addAllQuestionsFlag, noUpdateToConfigFlag, ignoreConfigBucketsAndChoices);
    seeder.startParsing()
    .then(() => {
