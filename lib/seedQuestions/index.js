...
class QuestionSeeder {
    ...
    _.map(this.questions, (question, questionsIndex) => {
      _.map(question.choices, (choice, choiceIndex) => {
        ...
        _.map(['common_text', 'hi_text', 'en_text','ma_text'], (prop) => {
          ...
          let value = choiceToken[0].raw==null?null:getHTMLFromMDTokens(choice.tokens[prop]);
  ...
  async uploadImages() {
    const allUploadPromises = _.map(this.questions, (question,
      ...
      const uploadPromises = _.map(['common_text', 'hi_text', 'en_text','ma_text'], (prop) => this.uploadImagesOfTokens(choice.tokens[prop], questionsIndex, choicesIndex, prop));
  ...
  getQuestionChoiceTokens(tokens, headingIndex) {
    ...
    _.map(headingIndexes, (Heading,index) => {
      ...
      }else if (Heading[0]==='en_text'||Heading[0]==='hi_text'||Heading[0]==='ma_text'){
        const rawtext = { raw:'', text:'' }
        const questionToken= tokens.slice(HeadingIndex+1,headingIndexes[index+1][1])
        for (var i of questionToken){

          rawtext.raw+=i.raw
          rawtext.text+=(i.text?i.text:i.raw )
        }
        currentTokens.push({...tokens[HeadingIndex + 1],...rawtext,tokens:[{type:'text',...rawtext}]});
      }
      else{
        currentTokens.push(_.clone(tokens[HeadingIndex + 1]));
