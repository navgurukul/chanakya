...
class QuestionSeeder {
  constructor(config, updateMDFiles, addAllQuestions, updateConfigFlag, ignoreConfigBucketsAndChoices) {
    this.config = config;
    this.questionFiles = [];
    this.regexs = {
  ...
  replaceImageLinks(imageLinks) {
    _.map(imageLinks, (link) => {
      if (link.prop == 'options') {
        const content = this.questions[link.questionsIndex].choices[link.choicesIndex].options[link.optionsIndex].text;
        ...
        const content = this.questions[link.questionsIndex].choices[link.choicesIndex].tokens[link.prop][link.tokenIndex].text;
  ...
  async uploadImages() {
    const allUploadPromises = _.map(this.questions, (question, questionsIndex) => _.map(question.choices, (choice, choicesIndex) => {
      const uploadPromises = _.map(['commonText', 'hiText', 'enText'], (prop) => this._uploadImagesOfTokens(choice.tokens[prop], questionsIndex, choicesIndex, prop));
  ...
  getQuestionChoiceTokens(tokens, headingIndex) {
    ...
    const options = this.parseChoiceOptions(questionChoice.tokens.options, choiceAttr.correctOption);
  ...
  static getQuestionDirValue() {
    if (process.argv.indexOf('--questionsDir') > -1) {
      ...
      try {
        const stat = fs.statSync(questionsDir);
        return questionsDir;
      } catch (e) {
  ...
  async bumpVersion() {
    ...
    let version;
    const questionIds = this.newQuestionIds.concat(this.updatedQuestionIds);
...
if (!module.parent) {
  ...
  // start the parsing
  const seeder = new QuestionSeeder(config, updateMdFlag, addAllQuestionsFlag, updateConfigFlag, ignoreConfigBucketsAndChoices);
  seeder.startParsing()
    .then(() => {
