...
class QuestionSeeder {
  constructor(config, updateMDFiles, addAllQuestions, updateConfigFlag,
    ...
    this.updatedquestion_ids = [];
    this.newquestion_ids = [];
    this.updatebucket_ids = [];
    this.newbucket_ids = [];
  }
  ...
  async startParsing() {
    ...
    // now `allquestion_ids` will have a list of all question IDs POST or PUT which were added
    await this.addQuestions(); // allquestion_ids
    ...
    _.map(this.finalQuestions, (obj) => {
      ...
      // tokensToMD.updateOptionsTable(obj.mainTokenIndexes.tokenIndex, obj.apiResponse.options);
    });
    ...
    this.latestVersion.questionIds = _.map(this.questionsGroupedByFile, (questionsArray) => {
      return _.map(questionsArray, (q) => q.apiResponse.id);
    });
  ...
  addQuestions() {
    const allResponses = _.map(this.finalQuestions, (obj, index) => {
      const object = obj;
      const isAdd = !object.question.id || this.addAllQuestions || this.updateConfigFile;
      if (isAdd) {
        delete object.question.id;
        _.each(object.question.options, (o, i) => {
          delete object.question.options[i].id;
        });
        return axios.post(`${CONSTANTS.apiBaseUrl}/questions`, object.question)
          .then((response) => {
            this.finalQuestions[index].apiResponse = response.data.data;
            this.newquestion_ids.push(response.data.data.id);
            return response.data.data.id;
          });
      } else {
        return axios.put(`${CONSTANTS.apiBaseUrl}/questions/${object.question.id}`, object.question)
          .then((response) => {
            this.finalQuestions[index].apiResponse = response.data.data;
            this.updatedquestion_ids.push(object.question.id);
  ...
  createFinalData() {
    ...
    _.map(this.questions, (question, questionsIndex) => {
      _.map(question.choices, (choice, choiceIndex) => {
        const finalQuestion = { ...choice.attrs }; // Create new question object
        finalQuestion.options = [];

        // Add schoolId to the question attributes
        finalQuestion.schoolId = choice.attrs.schoolId || null;
        // Set text properties safely
        _.map(['common_text', 'hi_text', 'en_text', 'ma_text'], (prop) => {
          const choiceToken = choice.tokens[prop];
          let value = (choiceToken && choiceToken[0]?.raw == null)
            ? null
            : getHTMLFromMDTokens(choiceToken || []);
          if (prop === 'common_text' || prop === 'en_text') {
            if (!value) {
              value = null;
            }
          }
        ...
        _.map(choice.options, (Option) => {
          ...
          if (finalQuestion.type !== CONSTANTS.questions.types.integer) {
            option.text = marked.parse(option.text || '');
          }
          option.text = (option.text || '').trim();
          finalQuestion.options.push(option);
        });
        // Push final question structure
        finalQuestions.push({
  ...
  async uploadImages() {
    const allUploadPromises = _.map(this.questions, (question,
      questionsIndex) => _.map(question.choices, (choice, choicesIndex) => {
        const uploadPromises = _.map(['common_text', 'hien_text', 'hi_text', 'en_text', 'ma_text'], (prop) => this.uploadImagesOfTokens(choice.tokens[prop], questionsIndex, choicesIndex, prop));
  ...
  getQuestionChoiceTokens(tokens, headingIndex) {
    ...
    // parse common text ,hinglish, english and hindi & options heading indexes
    let headingIndexes = {};
    _.map(tokens, (token, index) => {
      if (token.type === 'heading') {
        const token_text = token.text.toLowerCase();
        if (token.depth === 2) {
          if (token_text.includes('common text')) {
            headingIndexes.common_text = index;
          } if (token_text.includes('options')) {
            headingIndexes.options = index;
            questionChoice.mainTokenIndexes.options = tokens[headingIndex + index + 2];
            questionChoice.mainTokenIndexes.tokenIndex = headingIndex + index + 2;
          }
        } else if (token.depth === 3) {
          if (token_text.includes('english')) {
            headingIndexes.en_text = index;
          } if (token_text.includes('hinglish')) {  // Add your new condition here
            headingIndexes.hien_text = index;
          } if (token_text.includes('hindi')) {
            headingIndexes.hi_text = index;
          } if (token_text.includes('marathi')) {
            headingIndexes.ma_text = index;
          }
        }
      }
      // return index;
    ...
    _.map(headingIndexes, (headingPair, index) => {
      if (!headingPair || !Array.isArray(headingPair)) return;

      const [heading, HeadingIndex] = headingPair;

      const nextHeading = headingIndexes[index + 1];
      const nextIndex = nextHeading && Array.isArray(nextHeading) ? nextHeading[1] : tokens.length;

      const currentTokens = [];
      const tokenSlice = tokens.slice(HeadingIndex + 1, nextIndex);

      if (heading === 'common_text') {
        const nextToken = tokens[HeadingIndex + 1];
        if (nextToken && nextToken.type !== 'heading') {
          currentTokens.push(_.clone(nextToken));
        } else {
          currentTokens.push({
            type: 'paragraph',
            raw: '',
      ...
      } else if (['en_text', 'hien_text', 'hi_text', 'ma_text'].includes(heading)) {
        const rawtext = { raw: '', text: '' };
        for (const token of tokenSlice) {
          rawtext.raw += token.raw || '';
          rawtext.text += token.text ?? token.raw ?? '';
        }
        ...
        if (tokens[HeadingIndex + 1]) {
          currentTokens.push(_.clone(tokens[HeadingIndex + 1]));
        } else {
          currentTokens.push({
            type: 'paragraph',
            raw: '',
  ...
  static parseChoiceOptions(optionTableTokens, correct) {
    if (optionTableTokens.length !== 1) {
      QuestionSeeder.showErrorAndExit(`Something wrong with the options table. Please check.${optionTableTokens.length}`);
    }
    const token = optionTableTokens[0];
    if (!_.isEqual(_.map(token.header, (el) => el.text.toLowerCase()), ['option', 'values', 'id'])) {
      QuestionSeeder.showErrorAndExit(`The options table is not correct. Please check.${optionTableTokens.length}`);
    }

    // make a formatted options array
    const options = [];
    _.map(token.rows, (cell) => {
      options.push({
        text: cell[1].text,
        id: cell[2].text === 'null' ? null : Number(cell[2].text),
        correct: correct === Number(cell[0].text),
      });
    });
  ...
  async bumpVersion() {
    ...
    currentVersion = currentVersion.data.data || 0;
    ...
    this.newquestion_ids.concat(this.updatedquestion_ids); // question_ids
  ...
  static verifyConfigFile(questionsDir) {
    ...
    const configValidation = schemas.configSchema.validate(config);
...
if (!module.parent) {
  ...
  seeder.startParsing()
    .then(() => {
      ...
      console.log(`\nYou've added ${seeder.newquestion_ids.length} questions and updated ${seeder.updatedquestion_ids.length} already existing questions.\n`);
