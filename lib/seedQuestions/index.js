...
class QuestionSeeder {
    ...
    constructor(config, updateMDFiles, addAllQuestions, updateConfigFile, ignoreConfigBucketsAndChoices) {
        this.config = config;
        this.questionFiles = [];
        this.regexs = {
        ...
        // if active will make POST requests to add buckets.
    ...
    async startParsing() {
        ...
        if (!this.ignoreConfigBucketsAndChoices){
...
if (!module.parent) {
    ...
    if (updateToConfigFlag && IgnoreConfigBucketsAndChoices) {
        QuestionSeeder.showErrorAndExit("--noUpdateToConfig, & --ignoreConfigBucketsAndChoices cannot be active at once.")
    }
    ...
    // start the parsing
    let seeder = new QuestionSeeder( config, updateMdFlag, addAllQuestionsFlag, updateToConfigFlag, IgnoreConfigBucketsAndChoices);
    seeder.startParsing()
    .then(() => {
