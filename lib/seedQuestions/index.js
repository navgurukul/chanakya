...
class QuestionSeeder {
    ...
    constructor(config, updateMDFiles, addAllQuestions, noUpdateToConfigFlag, UpdateToConfigFlag, ignoreConfigBucketsAndChoices) {
        this.config = config;
        this.questionFiles = [];
        this.regexs = {
        ...
        this.noUpdateConfigFile = noUpdateToConfigFlag;
        // if active will update config file according to the user choice he want to update config file or not.
        this.UpdateToConfigFlag = UpdateToConfigFlag
    ...
    async startParsing() {
        ...
        // monitoring config file is not updated.
        if(this.noUpdateConfigFile) {
            console.log("Config file is not Updated.")
        }
        // updating config file once you run the script.
        if (!this.noUpdateConfigFile || this.UpdateToConfigFlag){
    ...
    static getnoUpdateConfigFileFlag() {
    ...
    static getUpdateConfigFileFlag() {
        if (process.argv.indexOf("--updateConfigFile") > -1) {
            return true
        }else {
            return false
        }
...
if (!module.parent) {
    ...
    let noUpdateToConfigFlag = QuestionSeeder.getnoUpdateConfigFileFlag()
    ...
    // both the update config and new bucket addition flag cannot be active at the same time.
    if (UpdateToConfigFlag && IgnoreConfigBucketsAndChoices) {
        QuestionSeeder.showErrorAndExit("--UpdateToConfig, & --ignoreConfigBucketsAndChoices cannot be active at once.")
    }
    ...
    if (noUpdateToConfigFlag && !readlineSync.keyInYN("Are you sure you don't want to update the config.json file with ther bucket Id and choice IDs?")) {
        noUpdateToConfigFlag = false
        console.log("You said you were not sure of continuing without updating config file.")
    }
    // check if user want to update config file or not.
    if(UpdateToConfigFlag && !readlineSync.keyInYN("Are you sure you want to update the config.json file while seeding questions?")) {
        UpdateToConfigFlag = false;
    }
    
    // checck if user want to add new QuestionBucket and questions choices or not.
    if (IgnoreConfigBucketsAndChoices && !readlineSync.keyInYN("Are you sure you want to do add the questionBuckets and question choices?")) {
        QuestionSeeder.showErrorAndExit("You said you were not sure of continuing without updating config file. with thier bucketId and question choices")
    }
    ...
    // start the parsing
    let seeder = new QuestionSeeder( config, updateMdFlag, addAllQuestionsFlag, noUpdateToConfigFlag, UpdateToConfigFlag, IgnoreConfigBucketsAndChoices);
    seeder.startParsing()
    .then(() => {
