...
const feedbackStudents = async (knex, to_assign) => {
  var students = await knex("students")
    .whereIn("stage", [
...
const interviewType = (knex, to_assign) => {
...
const studentByOwner = async (knex, ownerId) => {
  return await knex("students").where("current_owner_id", ownerId);
};
const updateOwnerInterviewCount = async (knex, count, id) => {
  return await knex("interview_owners")
...
const updateOwnerTableAndStudentTable = async (knex) => {
  ...
  var user = await knex("interview_owners");
  ...
  for (var i of user) {
    ...
    var type = await interviewType(knex, usertoassign);
    await updateStudentOwner(knex, students, i.id);
    type = type.map((types) => types.stage);
