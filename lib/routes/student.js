...
module.exports = [
  ...
  // getting github url by student Id, student id we will get from zoho
  {
  method:'GET',
  path:'/students/getGithubUrl/{student_id}',
  options:{
    description:'getting github url by student Id, student id we will get from zoho',
    tags:['api'],
    validate:{
      params:Joi.object({
    ...
    handler:async(request)=>{
        const {studentService}=request.services();
        try{
          const data = await studentService.getGithubUrl(request.params.student_id);
          logger.info('getting github url by student Id, student id we will get from zoho')
          return data;
        } catch (error) {
          return error;
        }
