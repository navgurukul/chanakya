...
module.exports = [
  ...
  //TODO:
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        if (["finallyJoined"].includes(request.payload.stage)) {
          await studentService.updateProvisionsData(
            request.params.studentId,
            "firstProvision"
          );
        }
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        const { studentService } = request.services();
        const { partner_id, name, provisions } = request.payload;
        ...
        const provisionsData = provisions
          ? await studentService.updateProvisionsData(
              request.params.studentId,
              provisions
            )
          : false;
        if (provisionsData) {
          return { data: "Successfully changed provisionsData" };
        }
