  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const { joined_compus_on } = request.payload;
        const joinedCampusOn = joined_compus_on
          ? await studentCampusService.UpdatejoinedCampusOn(
              request.params.studentId,
              joined_compus_on
            )
          : false;
        if (joinedCampusOn.success) {
          return { data: "Successfully changed joined_compus_on" };
        }
