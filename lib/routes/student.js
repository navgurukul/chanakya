...
module.exports = [
  ...
  {
    method: 'POST',
    path: '/student/updateStudentStage/{studentId}',
    options: {
      description: 'Update student stage by student id',
      tags: ['api'],
      validate: {
        params: Joi.object({
          studentId: Joi.number().integer().required(),
    ...
    handler: async (request) => {
      const { studentStage } = request.services();
      const studentDetails = await studentStage.updateOrInsertStudentStage(
        request.params.studentId,
        request.payload
      );
      return studentDetails;
