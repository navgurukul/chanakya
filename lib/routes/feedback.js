  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const studentDetails = await studentService.getStudentById(request.params.studentId);
        if (
          stage !== '' &&
          stage !== 'offerLetterSent' &&
          stage !== 'testFailed'
        ) {
          const notificationContent = await sendingSmsToStudents.prepareMailTemplate(
            request.params.studentId
          );
        ...
        logger.error(
          JSON.stringify({
            error: true,
            message: 'Cannot send notification for this stage',
          })
        );
        return { message: 'Cannot send for this stage' };
      },
    },
  },
  {
    method: 'POST',
    path: '/students/stageNotification/{id}',
    options: {
      description: 'Post notification by studentId.',
      tags: ['api'],
      validate: {
        params: Joi.object({
          id: Student.field('id'),
        ...
        payload: Joi.object({
          platform: Joi.array().required(),
        }),
      },
      handler: async (request) => {
        try {
          const { feedbackService, sendingSmsToStudents } = request.services();
          const statusCheck = await feedbackService.statusCheck(request.params.id);
          const stage = await sendingSmsToStudents.getStage(statusCheck[0]?.student_id);
          if (
            stage !== '' &&
            stage !== 'offerLetterSent' &&
            stage !== 'testFailed' &&
            stage !== 'enrolmentKeyGenerated' &&
            stage !== 'basicDetailsEntered'
          ) {
            const notificationContent = await sendingSmsToStudents.prepareMailTemplate(
              statusCheck[0]?.student_id
            );

            // **************** Sending notification via Email *******************

            if (request.payload.platform.includes('email') && statusCheck[0]?.state) {
              // Write your send email code here
              const sendEmailNotification = await sendingSmsToStudents.sendEmailToStudents(
                notificationContent,
                statusCheck[0]?.student_id
              );
              // Storing data in database with status of success or failure of email sending
              const notificationHistory = await feedbackService.addNotificationHistory(
                sendEmailNotification.toString(),
                request.params.id,
                statusCheck[0]?.student_id
              );
            ...
            logger.error(
              JSON.stringify({
                error: true,
                message: 'Cannot send the notification for this stage',
              })
            );
            return { message: 'Cannot send the notification for this stage' };
          }
        } catch (error) {
          logger.error(
            JSON.stringify({
