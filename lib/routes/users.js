...
module.exports = [
  ...
  {
    method: 'POST',
    path: '/users/email/sendOTP',
    options: {
      description: 'Send OTP to the user through email.',
      tags: ['api'],
      validate: {
        payload: Joi.object({
          email: Joi.string().email().required().lowercase(),
      ...
      handler: async (request) => {
        const { userService } = request.services();
        const otp = await userService.sendOTPOnEmail(request.payload.email);
        logger.info('Send OTP to the user through email.');
        return {
          data: otp,
        };
      },
    ...
    options: {
      ...
      handler: async (request) => {
        try {
          ...
          logger.info('Verify OTP for the user.');
          return {
