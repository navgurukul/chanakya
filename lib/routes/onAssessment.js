...
internals.validateEnrolmentKey = async (assessmentService, key) => {
  ...
  if (!enrolmentKey) {
    throw Boom.badRequest(
      "The Enrolment Key specified is wrong. Please check and try again."
    );
  }
  return enrolmentKey;
};
internals.ensureEnrolmentKeyStatus = (assessmentService, key) => {
  ...
  if (keyStatus === "testAnswered") {
    throw Boom.conflict(
      "This enrolment key has already been used to answer a test."
    );
  }
  return keyStatus;
};
...
module.exports = [
    ...
    options: {
      ...
      validate: {
        params: {
          ...
          uploadType: Joi.string().valid("profileimage"),
        },
        payload: {
          file: Joi.object()
            .type(Readable)
      ...
      handler: async (request) => {
        ...
        const res = await studentService.addOrUpdateImgUlr(
          fileS3URL,
      ...
      validate: {
        ...
        payload: {
          ...
          gender: Joi.string().valid(
            ..._.keys(CONSTANTS.studentDetails.gender)
          ),
          ...
          qualification: Joi.string().valid(
            ..._.keys(CONSTANTS.studentDetails.qualification)
          ),
          current_status: Joi.string().valid(
            ..._.keys(CONSTANTS.studentDetails.currentStatus)
          ),
          school_medium: Joi.string().valid(
            ..._.keys(CONSTANTS.studentDetails.school_medium)
          ),
          caste: Joi.string().valid(..._.keys(CONSTANTS.studentDetails.caste)),
          religon: Joi.string().valid(
            ..._.keys(CONSTANTS.studentDetails.religon)
          ),
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const questions = await assessmentService.getQuestionSetForEnrolmentKey(
          enrolmentKey
        );
  ...
  },
  {
    ...
    options: {
      description:
        "Saves the answers given by the student. Saves the end time of the test.",
      tags: ["api"],
      validate: {
        options: {
          allowUnknown: true,
      ...
      handler: async (request) => {
        ...
        const payload = _.object(
          _.map(_.pairs(request.payload), (x) => [
            x[0],
            x[1] === "" ? null : x[1],
          ])
        );
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const testResult = await assessmentService.ShowTestResult(
          request.params.enrolmentKey
        );
        if (testResult) {
          return testResult;
