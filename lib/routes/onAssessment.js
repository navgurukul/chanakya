  ...
  },
  {
    ...
    options: {
      ...
      validate: {
        ...
        query:Joi.object({
          school:Joi.number().integer().allow(null)
      ...
      handler: async (request) => {
        ...
        let questions;
        try {
          if(request.query.school!=null){
            const checkSchoolisValidOrNot = await assessmentService.checkSchoolisValidORnot(request.query.school);
            if(!checkSchoolisValidOrNot){
              throw Boom.badRequest('School is not valid');
            }
          }
          const enrolmentKey = await internals.validateEnrolmentKey(
            assessmentService,
