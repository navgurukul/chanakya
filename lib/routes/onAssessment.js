    ...
    options: {
      ...
      handler: async (request) => {
        ...
        try {
          ...
          questions = await assessmentService.getQuestionSetForEnrolmentKey(enrolmentKey);
        } catch (error) {
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        if (stage == 'offerLetterSent' || stage == 'pendingEnglishInterview' ||stage == 'testFailed'){
          const smsData = await sendingSmsToStudents.prepareSmsTemplate(enrolmentKey.student.id);
          const smsSend = await sendingSmsToStudents.sendSmsToStudents(smsData,enrolmentKey.student.id)
        }
