    ...
    options: {
      ...
      validate: {
        ...
        payload: {
          ...
          donor: Joi.array().items(Joi.number().integer().valid(CONSTANTS.donor.map((ids)=>ids.id))),
        },
      },
      handler: async (request) => {
        ...
        const changedDonor = donor ? await studentDonorService.updateOrInsertById(
          request.params.studentId, donor,
        ) : false;
        if (changedDonor.success) {
          return { data: "Successfully changed donor" };
        }
        throw Boom.badRequest(
