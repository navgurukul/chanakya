...
module.exports = [
  ...
  },
  {
    ...
    method: "PUT",
    path: "/students/changeCampus/{studentId}",
    options: {
      description: "Change student campus by studentId",
      tags: ["api"],
      validate: {
        params: {
          studentId: Joi.number().integer().required(),
        ...
        payload: {
          campus: Joi.string().valid(CONSTANTS.campus).required(),
        },
      },
      handler: async (request) => {
        const { campusService } = request.services();

        const changedStage = await campusService.updateById(
          request.params.studentId, request.payload.campus 
       );
        if (changedStage) {
