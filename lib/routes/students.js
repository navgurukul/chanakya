...
module.exports = [
  ...
  {
    method: 'GET',
    path: '/students/record_distribution',
    options: {
      description: 'get all students record distribution on basis of student satge',
      tags: ['api'],
      handler: async (request) => {
        const { studentService } = request.services();
        const record = await studentService.dashboardRecordDistribution();
        return { data: record };
      },
    },
  },
  {
    method: 'GET',
    path: '/students/owner_report',
    options: {
      description: 'get all owner record',
      tags: ['api'],
      handler: async (request) => {
        const { studentService } = request.services();
        const report = await studentService.ownerReport();
        return { data: report };
      },
    },
  },
