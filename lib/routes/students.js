...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        const { studentService, assessmentService, campusService } = request.services();
        ...
        const { stage  } = request.payload;
        const updated = Boolean(stage) ? await feedbackService.updateOrInsertByStudentStage(
          { student_stage : stage},
          request.params.studentId
        ): 0
        if (changedStage && updated > 0) {
    ...
    options: {
      ...
      validate: {
        ...
        payload: {
          campus: Joi.string().valid(CONSTANTS.campus),
          donor: Joi.string().valid(CONSTANTS.donor),
          // stage: Joi.string().valid(CONSTANTS.studentStages),
          sub_stage: Joi.string().valid(CONSTANTS.studentSubStage),
          email: Joi.string(),
        },
      },
      handler: async (request) => {
        ...
        const changedCampus = campus ? await campusService.updateOrInsertById(
          request.params.studentId, campus,
          ) : false
        ...
        const changedDonor = donor ? await donorService.updateOrInsertById(
          request.params.studentId, donor,
          ) : false
        ...
        const { feedbackService } = request.services();
        const { stage, sub_stage  } = request.payload;
        const updated = Boolean(sub_stage) ? await feedbackService.updateOrInsertByStage(
          { state : sub_stage},
          request.params.studentId
        ): 0
        if (updated > 0) {
          return {
            data: "Successfully changed sub_stage",
          };
        }
        ...
        const studentStatus = email ? await studentService.addOrUpdateEmail(
          email,
          request.params.studentId
        ) : false
