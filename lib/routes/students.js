...
module.exports = [
        ...
        options: {
            ...
            validate: {
                query: {
                    ...
                    questionSetId: Joi.string().description("Retreive's students details respective partners using there questionSetId")
                }
            },
            handler: async (request, h) => {
                const { studentService } = request.services();
                const { partnerIds, pageNum, pageSize, questionSetId } = request.query

                // get students details using there question set Id.
                if(questionSetId) {
                    return await studentService.findPartnerById(questionSetId);
                }
    ...
    // {
    //     method: 'GET',
    //     path: '/students/{questionSetId}/',
    //     options: {
    //         description: "Retreive's students details respective partners using there questionSetId",
    //         tags: ['api'],
    //         validate: {
    //             params: {
    //                 questionSetId: Joi.string()
    //             }
    //         },
    //         handler: async (request, h) => {
    //             const { studentService } = request.services();

    //             let students = await studentService.findPartnerById(request.params.questionSetId, request.params.paperSet);
