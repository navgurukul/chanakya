...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        const { studentService } = request.services();
        const percentile = await studentService
          .getStudentsPercentile(request.params.enrolmentKey, request.payload);
        return {
          data: percentile,
        };
      },
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        const { studentService } = request.services();
        const studentStatus = await studentService.addTagForOnlineClasses(request.params.studentId, request.payload.tag);
        if (studentStatus) {
          return { data: studentStatus };
        }
    ...
    options: {
      ...
      validate: {
        ...
        payload: {
          ...
          updateOrAddType: Joi.string().valid(...CONSTANTS.contactAddOrUpdateType).required(),
        }
      },
      handler: async (request) => {
        const { studentService } = request.services();
        const studentStatus = await studentService.addAndUpdateContact(request.payload, request.params.studentId);
        if (studentStatus) {
          return { data: studentStatus };
        }
        throw Boom.badRequest('The specified studentId is wrong. Please check and try again.');
  ...
  {
    method: 'POST',
    path: '/students/autoCalling',
    options: {
      description: 'get all students record distribution on basis of student stage and gender',
      tags: ['api'],
      validate: {
        payload: {
          contacts: Joi.array().min(1).required(),
        },
      },
      handler: async (request) => {
        const { studentService } = request.services();
        const connectCall = await studentService.autoCallToJoinOnlineClassForFailedStudent(request.payload.contacts)
        if (connectCall) {
          return {
