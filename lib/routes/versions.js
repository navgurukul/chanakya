...
module.exports = [
    ...
    },
    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                let buckets = request.payload.buckets;

                // check if the buckets array has unique bucket IDs
                let bucketIds = _.map(buckets, (b) => {
                    return b.bucketId;
                });
                if (bucketIds.length != buckets.length) {
                    throw Boom.badRequest("Buckets array should only have unique `bucketIds`");
                }

                // pass the data to the testing service to create a version
                const { testVersioningService } = request.services();
                let version = await testVersioningService.createAndMarkAsCurrent(
