...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        logger.info('Get the current version of the test.');
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        logger.info('Get all the versions available.');
      ...
      handler: async (request) => {
        ...
        if (bucketIds.length !== buckets.length) {
          logger.error(
            JSON.stringify({
              error: true,
              message: 'Buckets array should only have unique `bucket_ids`',
            })
          );
          throw Boom.badRequest('Buckets array should only have unique `bucket_ids`');
        }
        ...
        logger.info('Creates a new version and marks it as the current one.');
        return { data: version };
      },
    },
  },
