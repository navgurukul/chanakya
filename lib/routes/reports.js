    ...
    options: {
      ...
      validate: {
        query: {
          ...
          from_stage: Joi.string().required().valid(_.keys(commonConnfig.allStages)),
          to_stage: Joi.string().required().valid(_.keys(commonConnfig.allStages)),
        },
      },
      handler: async (request) => {
        ...
        const transitions = await metricsService
          .getStageTransitionMetrics(from_stage, to_stage, startDate, endDate);
