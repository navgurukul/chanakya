const CONSTANTS = require("../constants");
const Joi = require("joi");
module.exports = [
  {
    method: "POST",
    path: "/ownershedule",
    options: {
      description: "Create a new partner.",
      tags: ["api"],
      validate: {
          payload:{
            owner_id: Joi.number().integer(),
            from: Joi.date(),
            to: Joi.date(),
          }
      },
      handler: async (request) => {
        const { ownerScheduleService } = request.services();
        const data = await ownerScheduleService.create(request.payload)
        
        return { data };
      },
    },
  },
  {
    method: "GET",
    path: "/ownershedule",
    options: {
      description: "Get all sheduled owners.",
      tags: ["api"],
      handler: async (request) => {
        const { ownerScheduleService } = request.services();
        const data = await ownerScheduleService.get()
        
        return { data };
      },
    },
  },
//   {
//     method: "GET",
//     path: "/ownershedule/owner_id/{owner_id}",
//     options: {
//       description: "Get sheduled owners by id.",
//       tags: ["api"],
//       validate: {
//         params:{
//             owner_id: Joi.number().integer(),
//         }
//     },
//       handler: async (request) => {
//         const { ownerScheduleService } = request.services();
//         const data = await ownerScheduleService.getByOwnerId(request.params.owner_id)
        
//         return { data };
//       },
//     },
//   },
  {
    method: "GET",
    path: "/ownershedule/{id}",
    options: {
      description: "Get sheduled owners by id.",
      tags: ["api"],
      validate: {
        params:{
          id: Joi.number().integer(),
        }
    },
      handler: async (request) => {
        const { ownerScheduleService } = request.services();
        const data = await ownerScheduleService.getById(request.params.id)
        
        return { data };
      },
    },
  },
  {
    method: "PUT",
    path: "/ownershedule/{id}",
    options: {
      description: "Create a new partner.",
      tags: ["api"],
      validate: {
        params:{
          id: Joi.number().integer(),
        },
        payload:{
            owner_id: Joi.number().integer(),
            from: Joi.date(),
            to: Joi.date(),
        }
      },
      handler: async (request) => {
        const { ownerScheduleService } = request.services();
        const data = await ownerScheduleService.update(request.params.id,request.payload)
        return { data };
      },
    },
  },
  {
    method: "DELETE",
    path: "/ownershedule/{id}",
    options: {
      description: "Create a new partner.",
      tags: ["api"],
      validate: {
        params:{
          id: Joi.number().integer(),
        }
      },
      handler: async (request) => {
        const { ownerScheduleService } = request.services();
        const data = await ownerScheduleService.delete(request.params.id)
        return { data };
      },
    },
  },
]