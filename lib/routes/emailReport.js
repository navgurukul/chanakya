...
module.exports = [
  ...
  },
  {
    ...
    path: "/emailreport/{emailreportId}",
    options: {
      description: "get all email reports by Id",
      tags: ["api"],
      validate: {
        param: {
          emailreportId: Joi.number().integer().required(),
        },
      },
      handler: async (request) => {
        const { emailReportService } = request.services();
        const emailReportsById = await emailReportService.getEmailReportsById(
          emailreportId
        );
        return { data: emailReportsById };
      },
    },
  },
  {
