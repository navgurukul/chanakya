...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        const { partnerService } = request.services();
        const partner = await partnerService.meraki_link(
  ...
  {
    method: "GET",
    path: "/partners/joined_progress_made_card/{partnerId}",
    options: {
      description:
        "Get all partner students details who joined campus for progress made.",
      tags: ["api"],
      ...
      handler: async (request) => {
        ...
        const partner = await partnerService.student_progressMade_Cards(
          request.params.partnerId
        );
        return { data: partner };
      },
