  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const emailExist = await partnerService.checkPartnerEmailExist(partnerUser);
        if (emailExist){
          return {
            "error": "true",
            message: `partner user email already exist`
          }
        }
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const partnerUser = partnerDetails["partner_user"];
        const partnerUserUpdated = await partnerService.updatePartnerUser(partnerUser);
        if (partnerUserUpdated.error){
          return partnerUserUpdated
        }
        delete partnerDetails["partner_user"];
