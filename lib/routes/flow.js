...
module.exports = (server, options) => ([
    ...
    },
    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                let syncDatabase = new ChanakyaGSheetSync( request.services() );
    ...
    },
    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                const  studentService   = request.services();
                let syncSheet = new ChanakyaGSheetSync( request.services() );
    ...
    {
        method: "GET",
        path: '/general/syncWithGSheet/addStageTransitionHistory',
        options:{
            description: 'Syncs the enrolment keys & query callbacks to the sheet. One way sync from chanakya to sheet for recording stage transition history.',
            tags: ["api"],
            handler: async (request, h) => {
                let updateSheet = new ChanakyaGSheetSync( request.services() );
                await updateSheet.init();
                await updateSheet.updateSheetForStageTransitionHistory();
                return { 'success': true } 
            }
        }
    }
