...
const daysRange = require('../services/metrics')
...
module.exports = (server, options) => ([
    ...
    {
        method: 'GET',
        path: '/general/metrics/{dateRange}',
        options: {
            ...
            validate : {
                ...
                params: {
                    dayRange: Joi.number().integer().min(1).max(365)
                }
            },
            handler: async (request, h) => {
                const { metricsService } = request.services();
                return await metricsService.getMetrics(request.params.dayRange);
            }
        }
    }
]);
