...
module.exports = () => ([
  {
    ...
    options: {
      ...
      validate: {
        ...
        query: {
          ngcall_type: IncomingCall.field('call_type').required(),
      ...
      handler: async (request) => {
        ...
        if (request.query.ngcall_type === 'requestCallback') {
          // contact linked to the most most recent student
          const contact = await studentService.findOneByContact(mobile, true);
          if (contact) {
            await studentService.addIncomingCall(request.query.ngcall_type, contact);
          } else {
        ...
        } else if (request.query.ngcall_type === 'getEnrolmentKey') {
