    ...
    },
    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                // await metricsService.recordAllMetrics( moment().subtract(3, 'days') );
                // await metricsService.recordPendingMetrics();

                // new students (from partners)
                let date = moment().subtract(4, 'days');
                let newPartnerStudentsQuery = metricsService._newPartnerStudentsBaseQuery(date);
                let x = await metricsService.getMetricFromBaseQuery('newPartnerStudents', newPartnerStudentsQuery, date);

                // new students (from helpline)
                let newHelplineStudentsQuery = metricsService._newHelplineStudentsBaseQuery(date);
                let y = await metricsService.getMetricFromBaseQuery('newHelplineStudents', newHelplineStudentsQuery, date);
