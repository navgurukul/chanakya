    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (request.query.ngCallType == 'requestCallback') {
          const contact = await studentService.findOneByContact(mobile, true); // contact linked to the most most recent student
          if (contact) {
          ...
          const smsResponse = await exotelService.sendSMS(mobile, 'requestCallback');
    ...
    },
    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                await syncSheet.init();
                let studentStatus = await syncSheet.studentStatus(request.params.mobile);
