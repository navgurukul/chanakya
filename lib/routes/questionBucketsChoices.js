...
module.exports = [
  ...
  },
  {
    ...
    options: {
      description:
        "Create a new choice within a bucket of the given `bucket_id`",
      tags: ["api"],
      validate: {
        payload: Joi.object({
      ...
      handler: async (request) => {
        ...
        const bucket = await questionBucketService.findById(
          request.params.bucket_id
        );
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const choices = await questionBucketService.findChoicesBybucket_id(
          bucket.id
        );
        return { data: choices };
      },
